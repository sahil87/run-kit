---
type: memory
description: "The rk MCP surface — the `rk mcp` stdio server and the daemon's `/mcp` streamable-HTTP route over `internal/mcp`: the compiled-in policy table (a verb with no row is not a tool), Cobra-introspection schemas with the `Resolve` drift guard, the argv `Executor` (stdin plumbing, 45 s cap, `TMUX`/`TMUX_PANE` stripped), the three-tier JSON result parser, the ten seeded tools, the stateful SDK transport behind the `OriginPolicy` allowlist, the `rk doctor` rows, `rk url --mcp`, and the E2E harness."
---
# rk MCP Surface
