...
The `Run` (CLI, count≥1) path is unchanged and **always worktree mode** (its `EffectiveSpec` carries no `Where`, defaulting to worktree), so `rk riff` stays byte-identical.

### Isolation Mode (`Where`: worktree | checkout) — since `260714-q9cg`
...
### Worktree-Name Passthrough (`--worktree-name`) — since `260714-q9cg`

When `Where=="worktree"` and `WorktreeName != ""`, the engine prepends `--worktree-name <name>` to the `wt create --non-interactive --worktree-open skip <passthrough…>` argv (before the user passthrough) — the verified `wt create` flag that **skips wt's name prompt** and names the worktree (and therefore the `riff-<basename>` window). A blank `WorktreeName` yields the **byte-identical** pre-feature argv. The argv assembly is the pure helper `buildWtCreateArgs(spec, passthrough)` (table-tested); its `spec.Where != whereCheckout` guard keeps it self-contained (checkout never reaches `wt`, and `Spawn` already blanks a checkout-mode name; the CLI/fan-out paths pass no name). The name is validated **at the API layer** before it reaches argv — see § Security / Trust Boundary and [architecture](/run-kit/architecture.md) § API Layer.
...
- **Session** — `spec.Session` scopes *which session the window is created in* on the daemon path (a bare `-L`-only `new-window` with no attached client would land in the socket's ambient session, not the requested one). When `spec.Session` is non-empty: `new-window` carries `-t =<session>:` (`sessionTarget` → `tmux.ExactSessionTarget`, exact-match session form since `260717-hikh` — see [tmux-sessions](/run-kit/tmux-sessions.md) § Exact-Match Session Targets), `split-window`/`select-layout` target `=<session>:<name>` (`windowTarget`; the window-name part stays non-exact — riff uniquifies names within the session pre-spawn), and the collision probe reads `list-windows -t =<session>:`. Empty `Session` (the CLI path) leaves every call unscoped — byte-identical to pre-session behavior. `internal/riff` now imports `rk/internal/tmux` for `ExactSessionTarget` (no cycle). (This session threading was a review must-fix during `260713-sbk1`: the first extraction pass emitted `-L` only and never targeted the requested session; `260717-hikh` then hardened the bare `-t <session>` / `<session>:<name>` forms to the exact `=` forms because `new-window`'s `-t` is a *window* target that would otherwise let a window named like the session hijack the spawn; `TestBuildNewWindowCaptureArgs`/`TestBuildSpawnArgvs` now lock both the daemon `=`-bearing and the CLI unscoped argv shapes.)
...
**Exit-class conformance (`260717-rex1`)**: the sentinel constants were renumbered to the shll-toolkit exit-code convention (Principle 4 — usage errors are 2, operational failures are 1): `ExitValidation=2`, `ExitPrecondition=1`, `ExitSubprocess=3` (was `1`/`2`/`3` — validation and precondition swapped). The renumbering is a **numeric-value change only**; every consumer keys on the constant **identity** (`ExitValidation`, not `1`), so no CLI/HTTP mapping code changed. See [toolkit-standards](toolkit-standards.md) § principles P4.
...
Before each `tmux new-window`, `listWindowNames(ctx, spec)` runs `tmux list-windows -F '#W'` on the target server — **scoped to `spec.Session` (`-t =<session>:`, exact-match session form since `260717-hikh`) on the daemon path** so the collision probe reads the SAME session the window will be created in; **unscoped on the CLI path** — and `resolveWindowName(existing, base)` picks the first free name starting from `base`, then `base-2`, `base-3`, … (filling gaps). Base is always `riff-<worktree-basename>`. Accepted TOCTOU race between list and new-window — fallback is silent duplicate under tmux's default `allow-rename`.
...
### `IsFabProject` — fab-project detection for the tier gate (since `260714-gsmu-spawn-dialog-tier-gate-sidebar-entry`)
...
**Tier positional (since `260714-q9cg`)**: the `tier` parameter selects which fab tier the launcher resolves. A **non-empty** tier emits `fab agent <tier> --print` (the verified positional-tier form); an **empty** tier emits `fab agent --print` (no positional — **byte-identical to the pre-q9cg default-tier resolution**). The argv is built by the pure helper `fabAgentArgs(tier)` (`{"agent","--print"}` for empty, `{"agent",tier,"--print"}` for a named tier). The CLI passes `""` (its implicit default-tier behavior preserved); the HTTP handler passes `opts.Tier` (dialog Agent dropdown, validated via `validate.ValidateTier` before it reaches this argv). The existing `parseFabAgentOutput` single-line contract and silent `DefaultLauncher` fallback are unchanged for both forms.
...
- `app/backend/api/riff.go` — `handleRiffSpawn` / `handleRiffPresets` + `deriveRepoRoot`; the `where`/`worktreeName`/`tier` body decode + 400-before-subprocess validation ladder, and `ReadTiers` in the presets response **gated on `IsFabProject` (since `260714-gsmu`)** (web-UI frontend; see [architecture](/run-kit/architecture.md) § API Layer)
- `app/backend/api/router.go` — `RiffEngine` interface + `prodRiffEngine` wrapper + `NewTestRouterWithRiff` + route registration
- `app/backend/internal/fabconfig/fabconfig.go` — `ReadPresets`, `ReadPresetsOrdered`, `ReadTiers`/`BuiltinTiers`, `IsFabProject` (presets + tier NAMES + fab-project detection; `ReadSpawnCommand` + the `fabConfig` struct were deleted in `260703-w884`; `IsFabProject` added `260714-gsmu`)
- `app/backend/internal/validate/validate.go` — `ValidateWorktreeName` (shared `ValidateName` + riff-seam leading-`-`/`/`/space rejections) and `ValidateTier` (strict identifier, no leading `-`) — since `260714-q9cg`
