...
2. Wait for the status bar's **Connected** dot (the server subscription acked; the desktop sidebar footer is gone).
...
`@rk_pane_chat`, so the lens resolution falls back to tty and the terminals socket stays;
