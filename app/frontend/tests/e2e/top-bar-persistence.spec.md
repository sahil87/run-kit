...
2. Its center heading is **route-derived** and updates per route — Cockpit →
   `Server Cabin <server>` → back to Cockpit → `Board <board>` — including the
...
- Fully mocked — no tmux server, no `gh`, no real backend reads. The spec injects
  ...
  - `**/api/servers` → a single server `default` (one clickable server tile; one
    SSE attach).
  - `**/api/boards` → one board `myboard` (a clickable tile in the Cockpit BOARDS
    zone).
  ...
  - `**/api/host/metrics/stream*` → an open, empty event-stream (the Cockpit
    connection-dot source) so the cockpit bar mounts cleanly.
...
- **Heading aria-labels** carry no colon (the `PageType: name` colon is
  presentational): `Cockpit`, `Server Cabin default`, `Board myboard`.

## Tests

### `the persistent bar stays present and its heading updates across / → /$server → /board`

**What it proves:** across Cockpit → Server Cabin → Cockpit → Board navigation,
...
**Steps:**
1. `goto("/")`; assert the solo `Cockpit` heading and the `Run Kit home` brand
   crumb are visible.
2. Click the `default` server tile (scoped to the "Tmux servers" region). Assert
   URL `/default`, heading `Server Cabin default`, brand crumb visible, and the
   previous `Cockpit` heading is gone (count 0 — the mode is route-derived, not
   stacked).
3. **(Reload boundary)** Click the `Run Kit home` brand crumb — a full document
   ...
   remounts with the `Cockpit` heading, brand crumb visible, and the
   `Server Cabin default` heading gone.
...
### `an unmatched route falls back to the minimal cockpit heading (not the fuzzy-matched board param)`
...
falls back to the minimal `cockpit` heading instead of leaking the
...
`RootTopBar` forces the cockpit fallback.

**Steps:**
1. `goto("/board/x/y")`; assert the `Page not found` body is visible.
2. Assert the persistent bar shows the `Cockpit` fallback heading and its brand
   crumb is visible.
