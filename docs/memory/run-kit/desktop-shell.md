---
description: "The app/desktop Electron viewer shell — a BrowserWindow client of an rk serve URL that never auto-starts the daemon (child_process only for user-initiated daemon actions + detection). Terminology: 'host' = rk instance, 'server' = tmux. Covers hosts.json, welcome + 'This Mac' section, local-daemon control + GUI-PATH trap, last-path restore, the menu tier seam, the runkitShell bridge (frozen servers:* SPA boundary), and the quarantine-free CLI install/update auto-restart-at-the-swap flow."
type: memory
---
