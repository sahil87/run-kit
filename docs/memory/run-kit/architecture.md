...
1. **Top chrome** (`shrink-0, border-b border-border`) — `TopBar`, always-rendered two-line top bar with view-dependent content
2. **Main area** (`flex-1 flex flex-row min-h-0`) — sidebar + content column side by side
   - **Sidebar** (drag-resizable, default 220px, min 160, max 400, `shrink-0 overflow-y-auto`, hidden on mobile < 768px) — session/window tree. Width persisted to `localStorage` key `runkit-sidebar-width`. Full height of main area (top bar to viewport bottom). Session name click navigates to `/$session`; chevron toggles expand/collapse
   - **Content column** (`flex-1 min-w-0 flex flex-col`) — view-dependent content:
     - **Dashboard** (`/`) — `Dashboard` component with session card grid
     - **Project page** (`/$session`) — `ProjectPage` component with window card grid
     - **Terminal** (`/$session/$window`) — xterm.js + WebSocket relay
     - **Bottom bar** (`shrink-0, border-t border-border`) — terminal view only, width tracks content column (not full viewport)
...
**TopBar** (`app/frontend/src/components/top-bar.tsx`) — receives `view` prop (`"dashboard" | "project" | "terminal"`). Line 1 (`border-b border-border`): logo toggle + view-dependent breadcrumbs (logo only on Dashboard, `{logo} ❯ session` on Project, `{logo} ❯ session ❯ window` on Terminal) + connection indicator + `⌘K`/`⋯`. Line 2: view-dependent actions — `[+ Session]` only (Dashboard), `[+ Session]` + `[+ Window]` (Project), `[+ Session]` + `[Rename]` + `[Kill]` (Terminal). Right-side status and fixed-width toggle only on Terminal. Always rendered with `min-h-[36px]` (prevents layout shift).

**Sidebar** (`app/frontend/src/components/sidebar.tsx`) — session/window tree with split interaction targets. Session name click navigates to `/$session` (project page) via `onSelectSession` prop. Chevron (▶/▼) is a separate button for expand/collapse. Active session highlighted when on `/$session` route. Kill session (`onKillSession` prop) navigates to `/` after API success. Desktop: drag-resizable (default 220px, min 160, max 400, persisted to `localStorage`), collapsible via logo button. No footer (create session moved to top bar). Mobile (< 768px): drawer overlay from the left, triggered by logo button.

**BottomBar** (`app/frontend/src/components/bottom-bar.tsx`) — rendered only on Terminal view (`/$session/$window`); not rendered on Dashboard or Project page. Single row of `<kbd>` buttons: modifier toggles (Ctrl/Alt/Cmd with sticky armed state), arrow keys, Fn dropdown (F1-F12, PgUp/PgDn, Home/End), Esc, Tab, and compose toggle. All buttons 44px min-height for mobile touch targets. Sends ANSI escape sequences through the WebSocket ref. Modifier state managed by `useModifierState` hook.
...
Three-view model: Dashboard, Project page, and Terminal all render in the same content area slot. Chrome adapts per view via the `view` prop — no per-page chrome injection or separate layouts.
...
- **TanStack Router over React Router** — type-safe params and search params, built-in loader pattern. Three routes: `indexRoute` (`/`), `sessionRoute` (`/$session`), `sessionWindowRoute` (`/$session/$window`)
...
- **Derived chrome (not slot injection)** — Top bar receives a `view` prop (`"dashboard" | "project" | "terminal"`) and adapts breadcrumbs, Line 2 actions, and status display per view. Bottom bar only renders on terminal view. No `setLine2Left`/`setLine2Right`/`setBottomBar` setters. Split React Context preserved for performance (state vs dispatch).
...
- **Three-tier view model (Dashboard → Project page → Terminal) in a shared shell** — All three views render in the terminal area slot within the same app shell (top bar + sidebar + main area). Route detection via `useMatches()` in `app.tsx` derives a `view` variable from param presence. Chrome adapts per view (breadcrumbs, Line 2 actions, bottom bar visibility). No separate page layouts — views replace only the terminal content, not the chrome. (`260313-ll1j-dashboard-project-page-views`)
...
- **Kill redirects navigate immediately after API success (not after SSE update)** — After `killWindow` API succeeds, navigate to `/$session` (project page). After `killSession` API succeeds, navigate to `/` (dashboard). Redirects happen before SSE removes stale data. Kill window redirect wired via `onKillWindow` callback in `useDialogState` hook. Kill session redirect wired via `onKillSession` prop on Sidebar. (`260313-ll1j-dashboard-project-page-views`)
...
| 2026-03-14 | **Dashboard & Project page views** — three-tier URL hierarchy (`/` = Dashboard, `/$session` = Project page, `/$session/$window` = Terminal). New `sessionRoute` in TanStack Router. Route detection via `useMatches()` derives `view` prop. Chrome adapts per view (breadcrumbs, Line 2 actions, bottom bar conditional). Kill redirects: window → `/$session`, session → `/`. Sidebar session name navigates to project page. Auto-redirect on root removed. | `260313-ll1j-dashboard-project-page-views` |
