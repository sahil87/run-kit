...
| [Cron](cron.md) | rk-owned clock substrate — server-scoped cron entries fired into agents resolved at delivery time (role/session targets), idle-anchored backoff for the operator tick, orphan GC, the sidebar `CLOCK` section + watched-row overlay, the console's desktop Activity segment + Server page dashboard zones (the larger views), and the operator watchlist seam |
