...
| 5 | Confident | Update compose-strip.spec.md's `>_`-chip prose to `I` even though its `.spec.ts` is untouched | The companion doc factually describes the chip face; leaving it stale contradicts its purpose. Same-commit rule doesn't strictly bind (no `.spec.ts` change) — judgment call, easily reversed | S:70 R:90 A:85 D:75 |
