...
- **Rig table.** Four parallel bash arrays indexed by Playwright parallel index — `RIG_PORT`, `RIG_SERVER`, `RIG_FAMILY`, `RIG_STATE`. Rig *i* takes port triple `E2E_PORT + 3i` (Vite / backend +1 / stub +2), bounded to the 3400–3699 block; rig 0's triple is reclaimed and stepped by the existing logic, the extra triples are killed the same self-scoped way (`kill_triple`/`triple_busy` now take a base-port argument) but never stepped — a still-busy extra triple is a hard error, since the lane is CI-only.
