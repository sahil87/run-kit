...
**The payload is prompt text, never a command** — the `add` Long and the `cron` parent Long both state it: at fire time rk types the payload into the target agent's chat through the injection engine and presses Enter, exactly as if a person had typed it; it is never run as a command (to run a command, ask the agent to run it) (upt2).
...
- **AND GIVEN** the delivery history `0, 2m, 6m, 8m, 12m, 14m` replayed forward, **THEN** successive gaps read 4m, 8m, 16m, 30m, 30m
...
**Why**: the ◉ marks and the stale warning are projection, not control; mirroring the slug is cheaper than a second file format and fixes the same-term-two-meanings bug ("server-slug" resolved differently in the two repos) at its source.
...
**Decision**: the streak walk's seed is `largestRungWithGapAtMost(min, max, gap + seedSkew)` — the largest rung whose ladder gap fits inside the newest own-delivery gap plus one poll of jitter (`seedSkew = DefaultTickInterval`, 30s); a gap too small for any rung seeds a single-delivery streak.
