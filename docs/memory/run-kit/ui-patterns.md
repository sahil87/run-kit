...
Since `260701-5d0z-service-tiles-host-console`, `ServerListPage` renders a **SERVICES** zone (`<section aria-label="Services">`) as the LAST zone, below the TMUX SERVERS tile grid, inside the same scroll container. It surfaces the host's listening TCP ports as tiles — a listening port is a HOST property (owned by no tmux window/session), so `/` (the box-level console) is its home, mirroring the HOST HEALTH rationale. Read-only awareness view; the only action opens a port's UI in a tmux window (Constitution IV — nothing mutates host state directly).
...
- **Zero-server disabled state**: the button is `disabled` with `title="Create a server first"` when `servers.length === 0` (there is no target to host the iframe window). The tile still renders.
- **HTTP-likely gate (`NON_HTTP_PORTS` denylist)**: a listening TCP port carries no protocol label (read from `/proc/net/tcp`), so run-kit can't KNOW a port speaks HTTP without probing. Instead `isLikelyHttpPort(port)` gates the click via a small denylist of well-known non-HTTP ports — `NON_HTTP_PORTS = {5432, 3306, 6379, 27017, 5672, 11211, 9092, 2379, 25, 22}` (PostgreSQL, MySQL, Redis, Mongo, AMQP, Memcached, Kafka, etcd, SMTP, SSH) — where a proxied iframe would only ever show a broken frame. For those the button is `disabled` with `title="Not a web service"`; **the tile still renders** (a listening port is worth surfacing for awareness — only the click is disabled). A heuristic, not truth: an HTTP server on an unusual port stays clickable and a service on a nonstandard port slips through, both low-harm. `handleOpenInWindow` re-checks `isLikelyHttpPort` as a belt-and-suspenders guard for any non-disabled trigger (keyboard/programmatic).
