...
**Right cluster** (`flex items-center gap-3`, mode-invariant order): terminal-only controls (split-vertical, split-horizontal, close-pane — rendered only when `currentWindow` exists) → terminal-font `Aa` control (gated `mode !== "root" && mode !== "cockpit"` — neither has a terminal to size; board keeps it because its panes are terminals) → the three route-agnostic controls FixedWidth → Notification → Theme → the connection dot as the **right-most** element. Each control sits in a `hidden sm:flex` span (mobile hides the cluster). The connection dot (`role="status"`, green/gray) renders only when `mode !== "board" && mode !== "cockpit"` (i.e. terminal + root). With the brand anchor gone the dot is now the right edge — `260702-6m46` rewrote the icon-ordering comment to drop the old "pinned to the Run Kit anchor" rationale.
