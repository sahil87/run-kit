...
Since `260701-vshd-cockpit-host-console-home`, `ServerListPage` (`app/frontend/src/components/server-list-page.tsx`, route `/`) renders a **HOST HEALTH** zone ABOVE the tmux-server tile grid, inside the existing scroll container (`div.flex-1.min-h-0.overflow-y-auto`). This makes `/` — the only route that is about the BOX, not a session — a machine console. All three zones now render in top-to-bottom order **HOST HEALTH → SERVICES → server tiles** (zone 3 SERVICES shipped in `260701-5d0z-service-tiles-host-console`; see § SERVICES Zone on `/`). Read-only awareness view; nothing here mutates host state (Constitution IV).
...
Since `260701-5d0z-service-tiles-host-console`, `ServerListPage` renders a **SERVICES** zone (`<section aria-label="Services">`) BETWEEN the HOST HEALTH zone and the server-tile grid, inside the same scroll container. It surfaces the host's listening TCP ports as tiles — a listening port is a HOST property (owned by no tmux window/session), so `/` (the box-level console) is its home, mirroring the HOST HEALTH rationale. Read-only awareness view; the only action opens a port's UI in a tmux window (Constitution IV — nothing mutates host state directly).
