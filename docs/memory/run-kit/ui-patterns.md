...
| `/$server/$session/$windowId` | Terminal or Iframe | `AppShell` layout. The third segment is the stable tmux **window ID** (`@N`) — `$session` is retained for breadcrumb display only, NOT window identity. The router percent-encodes `@` in the path segment (`@2` → `%402`). Rendering branch: `rkType === "iframe"` renders `IframeWindow` (URL bar + iframe), otherwise `TerminalClient` + `BottomBar`. SSE connected. |
...
**Window identity is the tmux window ID (`@N`), not the index** (since `260529-chgz-window-id-routing`). The `$window` route param holds a stable `@N` string; all URL/API/relay/tmux layers address a specific window by this ID, while the window *index* is retained only for positional reorder/move. Old index-based bookmarked URLs (`/$server/$session/3`) are a **hard break** — they no longer resolve (no redirect shim; constitution §II — ephemeral URLs, no persistent state). See `architecture.md` § API Layer and `tmux-sessions.md` § Window addressing identity for the cross-layer contract.
...
1. **Mount-time alignment (one-shot)**. On the first SSE payload after a route mounts, `app.tsx` compares the URL's `$window` window ID (`windowParam`, a `@N` string) to `currentSession.windows.find(w => w.isActiveWindow)?.windowId`. If they differ, it fires exactly one `selectWindow(server, urlWindowId)` to align tmux to the URL — `selectWindow` now takes `(server, windowId)`, no session, since a window ID is a self-contained tmux target. Guarded by `hasAlignedToUrlRef: useRef(false)` plus `lastAlignedSessionRef` (resets the guard when the `${server}|${sessionName}` key changes — a fresh session route is effectively a fresh mount for the alignment contract). Subsequent route changes within the same session never re-fire alignment. This supports reload (URL=active is a no-op) and deep-link (URL≠active aligns tmux, then yanks any other tabs on the same server per multi-client convergence). Since the compared identifier is the stable `@N`, a reorder/kill that shifts indices never spuriously triggers re-alignment.

2. **URL writeback (continuous)**. A separate effect watches the SSE-derived `activeWindow` and calls `navigate({ replace: true, params: { ..., window: activeWindow.windowId } })` whenever `activeWindow.windowId !== windowParam`. No debounce — tmux truth wins always. The effect skips when `dialogOpenRef.current === true` to avoid focus-stealing re-renders mid-dialog (this gates the URL writeback only, not the underlying SSE-derived selection). Comparing `windowId` (not `String(index)`) is the side-effect bug fix from `260529-chgz-window-id-routing`: an index shift from reorder no longer produces a phantom writeback navigation to a different window.
...
**Sidebar clicks are pure mutations.** `navigateToWindow(session, windowId)` in `app.tsx` calls `selectWindow(server, windowId)` only — it does NOT call `navigate(...)` at click time. The URL update follows on the next SSE-driven snapshot via the writeback effect above (typically sub-500ms with the tmuxctl control-mode subscription; see `architecture.md` § tmux Control-Mode Subscription). Server-side overrides during a click (another agent or hook calling `tmux select-window` concurrently) resolve to whichever mutation tmux processed last — there is no client-side timer or debounce that prefers the click's intent over the external event. On mobile, `navigateToWindow` additionally closes the overlay sidebar after a destination tap.
...
All clients viewing the same server's session/window route converge on the same window — the per-server invariant. When the SSE `event: sessions` payload changes which window is `isActiveWindow` for the currently-viewed session, every browser tab on `?server=<same>` and viewing `/$server/$session/$windowId` navigates to the new window's `windowId` via `navigate({ replace: true })` (the URL writeback effect above).
...
- **URL input field** — shows current `rkUrl`, editable. On Enter, calls `updateWindowUrl(server, windowId, url)` via PUT `/api/windows/{windowId}/url`. On API failure, reverts input to the SSE-confirmed `rkUrl`. Styled: `bg-bg-card text-text-primary text-sm px-2 py-1 rounded border border-border`
...
**Session dropdown**: Lists all tmux sessions. Current session highlighted with `text-accent`. Selecting navigates to the session's first window by its window ID (`/{server}/{session}/{windows[0].windowId}`). First item: `+ New Session` action — triggers instant session creation (no dialog).

**Window dropdown**: Lists all windows in the current session. Current window highlighted (`w.windowId === currentWindow.windowId`). Selecting navigates to `/{server}/{session}/{windowId}`. First item: `+ New Window` action (creates new window in current session).
...
- **Snappy drag**: A `localOrder: string[] | null` state slice OVERRIDES `sessionOrder` during an active drag — `handleSessionReorderOver` computes the new order via `splice` on every dragover and writes it into `localOrder` for instant visual feedback. The override clears only on the NEXT incoming SSE `session-order` event (driven by a `useEffect` keyed on `sessionOrder` with a `sessionDragSourceRef.current === null` gate). Critically, the effect does NOT depend on the drag-source state itself: clearing `localOrder` synchronously on dragend would cause the sidebar to revert to the pre-PUT order during the SSE round-trip and then snap back when the broadcast arrives. The ref-based guard preserves the user's reordered view until the server confirms.
