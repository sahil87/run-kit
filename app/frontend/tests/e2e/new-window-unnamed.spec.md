...
auto-name to their folder): the sidebar `+ New Tab` action no longer pins a
...
- The `+ New Tab` seam is the session row's
  `button[aria-label="New tab in <session>"]`.

## Tests

### `+ New Tab omits the name from the create request (tmux auto-names)`

**What it proves:** Clicking the session's `+ New Tab` button sends
...
2. Navigate to `/<server>` and wait for the `Connected` indicator AND the
   session row to render (same-server gate — the sidebar subscribes SSE to
   the route's server only, so the create flow must stay on it).
