...
- [x] T011 Run `just build-frontend` — Vite bundle must include the three `.woff2` files and `@font-face` URLs must resolve to served paths. Inspect `app/frontend/dist/` to confirm. <!-- ran `just build` (full build; no build-frontend recipe exists); dist/fonts/ contains all three woff2; dist/index.html has preload link; build exit 0 -->
