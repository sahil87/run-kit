...
- [x] A-015 R14: `chrome-material.spec.ts` passes for dark, light and a warm light theme, asserting aside = top bar = status bar = chrome, tile = primary, chrome ≠ tile, xterm viewport = primary
...
| 6 | Confident | The e2e spec asserts the xterm background via `.xterm-viewport`'s computed `backgroundColor` when the element exists, and skips that one assertion otherwise | xterm.js sets the viewport background from the theme; the terminal may not be attached in every e2e rig state | S:55 R:90 A:75 D:70 |
