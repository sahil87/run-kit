---
description: URL structure, three-way server route guard + create-server/server-gone lifecycle, dashboard/sidebar/boards components, shared PR-status line + unified StatusDot (one lifecycle-journey dot — PR > fab > tmux precedence, hue=phase / shape=status — reused on sidebar row, dashboard card, pane-panel header), Web Push opt-in (terminal-themed Cmd+K palette entry, not a bell), chrome, theme system, user-controllable global terminal font size (ChromeContext preference + top-bar combo + palette actions), adaptive terminal write flush + deferred per-connection reset + (server, owning session)-keyed relay connection identity, keyboard shortcuts, sidebar tree keyboard navigation + W3C-APG tree ARIA, component conventions
type: memory
---
# run-kit UI Patterns
...
> Spec: `docs/specs/status-dot.md` (+ `docs/img/status-dot-matrix.svg`) is the canonical reference for the full matrix and the palette rationale.
...
**WindowPanel** (`app/frontend/src/components/sidebar/status-panel.tsx`) — collapsible panel with `title="Pane"`, `storageKey="runkit-panel-window"`, `defaultOpen={true}`. The `headerRight` slot renders `<StatusDot win={win} /> {win.name}` (in a `flex items-center gap-1.5 truncate text-text-secondary font-mono` span) — the unified status dot (see § Status Dot) leads the header so the window's lifecycle status (fab phase / PR / activity, since `260615-0hsz-status-dot-lifecycle-journey`) stays glanceable even when the panel is collapsed. Displays per-pane metadata rows: `tmx` (pane index + ID), `cwd` (shortened path), `git` (branch), `fab` (change ID + slug + stage) or `run` (process name), and `agt` (agent state). No window selected -> "No window selected" in secondary text. `StatusPanel` is exported as a deprecated alias for backward compatibility.
