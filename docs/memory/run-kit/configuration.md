...
`internal/homemigrate`'s `Migrate()` performs the one-time home copy at release daemon start: it is called at the top of `serve`'s RunE — before `config.Load()` and `tmux.EnsureConfig()` read anything — followed by `tmux.RefreshDefaultConfigPath()` so the post-publish boot classifies and refreshes the new home's tmux.conf (`tmux.DefaultConfigPath` otherwise resolves at init time). Dev builds (`version == "dev"` — `just dev`/air, e2e rigs) skip it entirely, as does any run under `RK_CONFIG_DIR`; `rk daemon start`/`restart` never call it (the virtual port pin, § Port Policy, covers their pre-serve port resolution), and no generic CLI invocation triggers it.
