...
**Why**: the old format ran `#()` git shell jobs (`git rev-parse`, `git branch`) per pane on every border draw. On a `split-window` where a window has two size-participating clients of different sizes (the common case: an agent's own terminal plus the run-kit web viewer), the resize renegotiation re-fires those jobs in a burst that wedges the whole tmux server ~40–64s, freezing every client and the dashboard. Caching the values as options the daemon computes (via the shared `internal/gitinfo` resolver, so the border matches the sidebar) removes every draw-time fork, so the storm cannot occur regardless of how many resizes happen. Stamping lives on the snapshotter tick because it is subscriber-independent — a native-terminal-only user with no browser open still gets a populated border.
**Rejected**: stamping from the SSE poll (browser-gated — blanks the border when no web client watches the server); a `window-size`/`aggressive-resize` change to suppress the renegotiation (alters the multi-viewer sizing behavior run-kit is built around); dropping the branch/badge from the border entirely (loses information users rely on).
