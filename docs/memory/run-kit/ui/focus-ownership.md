...
The **stateful tile chords** (⌘1/⌘2/⌘3, `lib/tile-chord.ts` — § [keyboard-and-palette](/run-kit/ui/keyboard-and-palette.md)) obey the same recording rules: the focus arm rides the `layoutFocusTileRef` seam, so ⌘1-to-tty records `tty` through the seam's `recordTtySlot` while ⌘2/⌘3 record NOTHING (a chord hop must never teach the guard a code preference — exactly the focus-hop rule). The show-hidden arm arms the **per-kind landing flag** (`focusOnLandingRef` in `app.tsx`, consumed by the layout-keyed effect — one flag holding a `SurfaceKind`, shared by every open-then-focus path) and the hide arm returns focus through `restoreFocus` (§ Restore Router). The other family members record through their own genuine-focus seams or not at all: ⌘I records `compose` because it genuinely focuses the strip textarea, whose existing `onFocus` recorder fires (no new recording call), and ⌘B records NOTHING — the sidebar is chrome, not a per-window `FocusKind`.
