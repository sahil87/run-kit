...
1. **Mount-time alignment (one-shot)**. On the first SSE payload after a route mounts, `app.tsx` compares the URL's `$window` to `currentSession.windows.find(w => w.isActiveWindow)?.index`. If they differ, it fires exactly one `selectWindow(server, session, Number(urlWindow))` to align tmux to the URL. Guarded by `hasAlignedToUrlRef: useRef(false)` plus `lastAlignedSessionRef` (resets the guard when the `${server}|${sessionName}` key changes — a fresh session route is effectively a fresh mount for the alignment contract). Subsequent route changes within the same session never re-fire alignment. This supports reload (URL=active is a no-op) and deep-link (URL≠active aligns tmux, then yanks any other tabs on the same server per multi-client convergence).

2. **URL writeback (continuous)**. A separate effect watches the SSE-derived `activeWindow` and calls `navigate({ replace: true })` whenever `String(activeWindow.index) !== windowIndex`. No debounce — tmux truth wins always. The effect skips when `dialogOpenRef.current === true` to avoid focus-stealing re-renders mid-dialog (this gates the URL writeback only, not the underlying SSE-derived selection).
...
**Sidebar clicks are pure mutations.** `navigateToWindow(session, windowIdx)` in `app.tsx` calls `selectWindow(server, session, windowIdx)` only — it does NOT call `navigate(...)` at click time. The URL update follows on the next SSE-driven snapshot via the writeback effect above (typically sub-500ms with the tmuxctl control-mode subscription; see `architecture.md` § tmux Control-Mode Subscription). Server-side overrides during a click (another agent or hook calling `tmux select-window` concurrently) resolve to whichever mutation tmux processed last — there is no client-side timer or debounce that prefers the click's intent over the external event. On mobile, `navigateToWindow` additionally closes the overlay sidebar after a destination tap.

**Sidebar selection follows `isActiveWindow`.** `WindowRow.isSelected` derives from `currentSessionName === session.name && ((!ghost && win.isActiveWindow) || currentWindowIndex === String(win.index))` in `app/frontend/src/components/sidebar/index.tsx`. The `isActiveWindow` term (set by the backend `WindowInfo.IsActiveWindow` from tmux's `#{window_active}`) is the primary driver — this is what makes the sidebar yank in lockstep with `tmux select-window`, including external switches and `rk riff` window creation. The URL-based `currentWindowIndex === String(win.index)` clause is preserved as a fallback for two narrow cases: (a) ghost windows that exist mid-creation before the SSE snapshot includes them, and (b) the initial render before the first SSE payload arrives. Ghost windows specifically exclude the `isActiveWindow` term (`!ghost && win.isActiveWindow`) because the backend hasn't observed them yet.

### Multi-Client Convergence

All clients viewing the same server's session/window route converge on the same window — the per-server invariant. When the SSE `event: sessions` payload changes which window is `isActiveWindow` for the currently-viewed session, every browser tab on `?server=<same>` and viewing `/$server/$session/$window` navigates to the new window via `navigate({ replace: true })` (the URL writeback effect above).
