...
// enrichWindow enriches a single window with fab state.
func enrichWindow(win *tmux.WindowInfo, projectRoot string) {
	fabPath := win.WorktreePath
	if fabPath == "" {
		fabPath = projectRoot
	}
	change := fab.GetCurrentChange(fabPath)
	if change != "" {
		win.FabStage = change
		progress := fab.GetStatus(fabPath)
		if progress != "" {
		...
		go func(idx int, sd sessionData) {
			...
			if projectRoot != "" && hasFabKit(projectRoot) {
				...
				for j := range sd.windows {
					...
					go func(wIdx int) {
						defer winWg.Done()
						enrichWindow(&sd.windows[wIdx], projectRoot)
					}(j)
				}
				winWg.Wait()
			}
...
// ProjectRoot derives the project root from a session's first window.
