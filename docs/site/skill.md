...
- `rk tab layout [@N] [L|--add S|--rm S|--promote S|--cycle]` — read or mutate the tab's surface layout, a split tree (`h(tty,web)`, …; a legacy `split-h:tty,web` string still parses and rewrites to the tree); unset reads as `tty`.
