...
- **Codex** requires its native trust review: after `run-kit agent setup` writes `hooks.json`, open `codex`, run `/hooks`, and trust the run-kit entries. Until then they are installed but inactive — by Codex's design, not a run-kit limitation.
...
Hooks fire for sessions launched by hand in any shell — the integration does not depend on run-kit owning the launch (`rk riff`-spawned agents are simply the common case).
