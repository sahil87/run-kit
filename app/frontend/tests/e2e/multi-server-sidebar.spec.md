...
- `beforeAll` creates a session on the primary tmux server
  ...
  server (`rk-test-e2e-<token>-msb-<pid>-<suffix>`, built from the
  `TMUX_FAMILY` anchor, where `<pid>` is the Playwright `process.pid` so the
  automatic post-sweep can parse it and the family-anchored teardown glob
  reaps it) with its own session, each containing one named
