...
- **Dialog control conventions** — every dialog button composes the shared `WIDE_BTN_BASE` floors (28px fine / 40px coarse — the `--ctl-h-bar`/`--ctl-h-bar-coarse` lockstep) and every dialog text input carries `INPUT_COARSE` (40px coarse min-height; fine sizes unchanged) plus `INPUT_FOCUS` (the one live-input focus treatment, `outline-none focus:border-accent-green`), all from `components/controls.ts` ([visual-design](/run-kit/ui/visual-design.md) § Global Control-State Rules → Dialog and input constants). Destructive confirms compose the shared pair — `CONFIRM_NEUTRAL` (cancel/safe arm) / `CONFIRM_DANGER` (the destructive arm on the `signal-red` token) — consumed by the sidebar kill dialog (`sidebar/kill-dialog.tsx`), the board kill trio (`board/board-page.tsx` — "Unpin instead"/"Cancel" neutral, "Kill" danger), and the server dialogs (`server-dialogs.tsx` — kill, force-kill, adopt); call sites add only layout (`flex-1`/`w-full`). Dialog/panel button disabled states follow the unified recipe (`disabled:opacity-40 disabled:cursor-not-allowed` + hover neutralized back to the rest arm). `window-note-prompt.tsx`'s submit disables only while a submit is in flight — an empty submit is a valid note-clear, so emptiness never disables (unlike its `session-name-prompt.tsx` twin, whose collision/empty guards block submit). (260906-3i9e)
...
- **Non-protected target** — the plain two-button confirm ("Kill server <name> and all its sessions? This cannot be undone."), zero drift from the shared confirm path (the `CONFIRM_NEUTRAL`/`CONFIRM_DANGER` constants — § Component Conventions → Dialog control conventions).
