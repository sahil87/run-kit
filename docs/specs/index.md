...
| [Voice Round-Trip Studies](../wiki/voice-round-trip-studies.html) | Interactive HTML design study for the voice round-trip (push-to-talk dictation + spoken replies) — the playable 6-stage loop (listen → transcribe → deliver → work → speak back → ask back), the voice HUD card anatomy (who-header, transcript body, meta chip row; cyan/green/amber lanes), and the wire-log panel over the existing seams. Self-contained; open in a browser |
