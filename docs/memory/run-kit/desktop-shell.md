---
description: "The app/desktop Electron viewer shell — a BrowserWindow client of an rk serve URL that never auto-starts the daemon and never auto-updates (child_process only for user-initiated rk actions + read-only detection). Terminology: 'host' = rk instance, 'server' = tmux. Covers hosts.json, welcome + 'This Mac', local-daemon control + GUI-PATH trap, last-path restore, the menu tier seam + Restart-to-Update item, the runkitShell bridge (frozen servers:*), and quarantine-free CLI install/update."
type: memory
---
...
Package tests run via `node --test "dist/**/*.test.js"` after compile — the store, window-open-policy, local-daemon, and update-check modules are electron-free precisely so Node's built-in runner covers them without adding a test dependency. Compiled test files are excluded from packaging (`files: ["dist/**", "!dist/**/*.test.js"]`).
...
Verification split: compile, `tsc --noEmit`, node:test (store, window-open policy, local-daemon pure logic, update-check pure logic), and vitest (`shell.test.ts`) all run on Linux. Playwright does not cover the Electron shell at all, so the pure-module suites plus the compile gates are the automated surface. Hardware-only items, per platform:
