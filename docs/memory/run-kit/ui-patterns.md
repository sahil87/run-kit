---
description: URL structure, three-way server route guard + create-server/server-gone lifecycle, dashboard/sidebar/boards components, shared PR-status line, chrome, theme system, keyboard shortcuts, component conventions
---
# run-kit UI Patterns
...
`app/frontend/src/components/pr-status-line.tsx` — `PrStatusLine({ win })`, a shared one-line live PR-status display reused by the sidebar `WindowRow` and the dashboard window cards (`260610-596o-pr-status-sidebar`). A single component keeps the gate, glyphs, summary text, and color tokens identical across both surfaces.

**Change-bound gate**: returns `null` unless `win.fabChange && win.prNumber` (early return) — so a scratch window (a PR number but no bound change, or vice versa) renders nothing. This is the same gate the backend enforces before attaching `prState`/`prChecks`/`prReview`/`prIsDraft` (see `architecture.md` § PR-Status SSE Join), so the four status fields are only populated when the gate already holds; the frontend gate is the display-side mirror.

**Layout**: `PR #<n> <state-glyph> <state>[ (draft)] · <checks/review summary>`. The state glyph comes from `stateGlyph(prState)` (`merged`→`✓`, `closed`→`✗`, else `●`). The summary (`summaryText`) joins the non-`none` parts with ` · `: `checks <prChecks>` and `review: <prReview with underscores → spaces>` (e.g. `checks pass`, `review: changes requested`). `prIsDraft` appends ` (draft)` to the state.

**`PR #<n>` link**: when `win.prUrl` is set it is an `<a href={prUrl} target="_blank" rel="noopener noreferrer">` whose `onClick` calls `stopPropagation` so clicking the link opens the PR in a new tab but does NOT select/navigate the window (nor trigger the row's refresh). When `prUrl` is absent it degrades to a plain `<span>`. Because the link is an `<a>`, the dashboard had to move it outside the card `<button>` (see § Dashboard window cards) and the sidebar places it outside the row `<button>` too — no `<a>`-in-`<button>`.

**Click-to-refresh**: clicking the line itself (the `role="button" tabIndex={-1}` wrapper `div`, but NOT the `<a>` link) calls `e.stopPropagation()` then `void refreshPrStatus().catch(() => {})` — a best-effort on-demand `POST /api/pr-status/refresh`. It never blocks and swallows errors (`gh` may be absent/unauth server-side); the refreshed status arrives via the next SSE `sessions` payload. `title="Refresh PR status"`. `data-testid="pr-status-line"` / `data-testid="pr-status-link"`.
...
- `window-row.tsx` — `WindowRow`; pure presentational; renders a single window row (activity dot, name, fab stage, duration, kill button); handles drag-and-drop and inline rename display; all event handlers passed as props. Below the name/`fabStage` row it renders a `<PrStatusLine win={win} />` (non-ghost only; gate lives inside the component — change-bound windows with a PR only) in a `pl-[18px] pr-11` indented sibling div, OUTSIDE the row `<button>` so the PR `<a>` link is valid HTML (`260610-596o-pr-status-sidebar`; see § PR Status Line)
