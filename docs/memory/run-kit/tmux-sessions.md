...
- `SelectWindow(windowID, server)` — selects a window on the specified server by its stable window ID (`@N` is a self-contained `-t` target; no session:index string — since `260529-chgz-window-id-routing`)
- `ResolveWindowSession(ctx, server, windowID)` — returns the owning session name for a window ID via a targeted `display-message -t <windowID> -p '#{session_name}'` (O(1)); used by the relay (to build the grouped ephemeral) and `ProjectRoot`. Errors (or empty result) mean "window not found"
...
- `MoveWindow(windowID, dstIndex, server)` — reorders a window within its own session. Source addressed by stable `windowID`; destination remains positional. Resolves the source's current index from the ID (via `resolveWindowSessionIndex`, a `display-message -p '#{session_name}\t#{window_index}'` lookup), then bubble-swaps via adjacent `swap-window` calls (insert-before semantics). tmux preserves the window's ID across the swaps
