...
A server enters the SSE poll set exactly once — when a browser opens `GET /api/sessions/stream?server=<name>`, `addClient` adds it to `h.clients` and re-spawns the poll goroutine if it had exited (the `!h.polling` guard). The hub's per-tick work-list is the snapshot of `h.clients` keys. Since `260603-gs2t-reap-dead-tmux-servers-sse` a server leaves the poll set via **two** paths, not one:

1. **Last-client-disconnect** — `removeClient` deletes the server from `h.clients` when its final SSE client drops; the next tick observing zero clients lets the goroutine stop.
2. **Dead-server reap (new)** — when `FetchSessions` returns an error matching `tmux.IsServerGone`, the server is reaped even while clients are still connected. Killing a tmux server does NOT disconnect the browser's `EventSource`, so before this change the dead socket lingered in `h.clients` and every ~2.5s tick re-shelled `tmux -L <name> list-sessions` against the gone socket, logging a `slog.Warn("SSE poll error")` drumbeat forever (the `utils`/runaway-polling incident).

**Reap mechanics**: `sseHub.poll` collects gone servers into a **loop-local `deadServers` slice** during the per-server iteration — the fetch-error branch now splits on `tmux.IsServerGone(err)`: a gone error logs at **Info** (`"SSE: tmux server gone, reaping from poll set"`) and appends to `deadServers`; a non-gone error keeps the existing `slog.Warn("SSE poll error", ...)` and is **not** reaped. Both still `continue`. After the per-server loop completes (before the metrics broadcast), a single `h.mu.Lock()` block emits one `event: server-gone\ndata: {}\n\n` frame to each dead server's currently-registered clients, then deletes the server from **every** per-server map so no stale state leaks into a later re-registration: `h.clients`, `h.cache`, `h.previousJSON`, `h.previousRealSessions`, `h.orderBootstrapAttempts`, `h.previousOrderJSON`, plus the loop-local `perServerGen` and `eventDrivenServers` maps.

**Concurrency contract** (load-bearing): the work-list is snapshotted under `RLock`, dead servers are collected into the loop-local slice *during* iteration, and all deletes happen *after* iteration under one write lock — never deleting from a map mid-range over its snapshot, never holding the write lock across `FetchSessions`. **Re-registration is free**: when the last client for a server is reaped, the next tick sees zero clients and the goroutine exits; a later `addClient` (e.g. the user restarts the server and navigates back) re-adds it and re-spawns the goroutine. The frontend reaction to `server-gone` is in `ui-patterns.md` § Server-Gone Reap (SSE event + onerror fallback).
