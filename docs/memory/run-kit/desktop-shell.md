---
description: "The app/desktop Electron viewer shell — a BrowserWindow client of an rk serve URL that never auto-starts the daemon and never auto-updates (child_process only for user-initiated rk actions + detection). Terminology: 'host' = rk instance, 'server' = tmux. Covers hosts.json, welcome + 'This Mac', local-daemon control + GUI-PATH trap, last-path restore, the hidden titlebar + page-drawn strip, the dock/taskbar badge, the menu tier seam, the runkitShell bridge, and quarantine-free CLI install/update."
type: memory
---
...
└── src/
    ├── main.ts             # lifecycle, BrowserWindow, security wiring, IPC, welcome ↔ host routing, last-path capture, local-daemon control, update detection + detached update spawn
    ...
    ├── strip.ts            # titlebar-strip pure logic (electron-free): STRIP_HEIGHT_PX, symbolColorFor, fallbackStripCss, shouldInjectFallbackStrip
...
Package tests run via `node --test "dist/**/*.test.js"` after compile — the store, window-open-policy, local-daemon, update-check, strip, and badge modules are electron-free precisely so Node's built-in runner covers them without adding a test dependency. Compiled test files are excluded from packaging (`files: ["dist/**", "!dist/**/*.test.js"]`).
...
The shell remembers where each host was left, so switcher-accelerator switching (⌥⌘1–9 on mac, ⇧Ctrl+1–9 on win/linux) and cold start land on the route the user was working in rather than the SPA root.
...
The window has **no native titlebar**: the page's top edge is the visible titlebar. `titleBarStyle` is `"hiddenInset"` on darwin (traffic lights composite over the page-drawn band) and `"hidden"` elsewhere plus a `titleBarOverlay: { color, symbolColor, height: STRIP_HEIGHT_PX }` so the native `─ ▢ ✕` controls draw over the band's right end. `STRIP_HEIGHT_PX = 28` is one value shared by the overlay height and the SPA's strip, so the controls composite over the strip exactly. This is a *strip*, not a titlebar merge — the SPA's top bar is untouched below it, and the merge stays possible on the same foundation.

**The SPA draws the strip; the shell provides no color API.** The band is a 28px accent-tinted band the SPA renders above its top bar, gated on `isShell()` (see [ui-patterns](/run-kit/ui-patterns.md) § Desktop-Shell Titlebar Strip). The band is a drag surface with **exactly one** no-drag island: its centered host-name label is the SPA's host-switcher dropdown trigger, which hands off to `switchToHost` through the existing `servers.switch` channel — a renderer-side feature that needs no shell change (§ Design Decisions → The titlebar is colored by a page-drawn strip). The shell's only color seam is the page's **existing `theme-color` meta**: `webContents.on("did-change-theme-color")` records the observed color and, on non-darwin, calls `win.setTitleBarOverlay({ color, symbolColor: symbolColorFor(color), height })`. Because that meta is already accent-aware end-to-end — including the pre-paint localStorage echo in `index.html` — the native controls tint with the instance accent without a single new IPC channel. `symbolColorFor` is a pure WCAG-luminance pick between the two standard text hexes (`#e5e7eb` light / `#111827` dark; unparseable input reads as dark). Linux window-controls-overlay support is partial, so the `setTitleBarOverlay` call is wrapped in a try/catch and degrades silently. macOS returns early — traffic lights are OS-drawn and take no color.

**Version skew is covered by injected CSS, not by probing.** An older SPA under this shell would draw no strip and leave the window with **no drag surface at all**. On `did-finish-load`, when `shouldInjectFallbackStrip(url, registeredOrigins())` passes (an http(s) URL whose origin is registered — the welcome `file://` page and foreign origins get nothing), the shell `insertCSS`es `fallbackStripCss(lastThemeColor)`: a fixed 28px `body::before` band with `-webkit-app-region: drag` plus matching `body` top padding, backgrounded by the last observed theme color (hex-validated before interpolation, falling back to `DEFAULT_STRIP_COLOR = "#0f1117"`). Every selector is keyed on `html:not(.rk-shell-strip)`, and the SPA sets `rk-shell-strip` on `<html>` while its strip is mounted — so the injected rules self-disable the moment the real strip exists. CSS is live, which is why injecting unconditionally on every registered-host load is safe and needs no `executeJavaScript` probe (§ Design Decisions → The fallback strip is self-disabling CSS).
...
`STRIP_HEIGHT_PX`, `STRIP_MARKER_CLASS`, `DEFAULT_STRIP_COLOR`, `symbolColorFor`, `fallbackStripCss`, and `shouldInjectFallbackStrip` live in `src/strip.ts` — the fifth electron-free module — with `strip.test.ts` covering the symbol-color matrix, the marker-keyed selectors, the hex-validation fallback, and the injection predicate (registered origin → inject; welcome `file://`, foreign origin, unparseable URL → no inject).
...
**Scope is the window's own host.** The count covers everything the connected instance's SSE stream carries and nothing else — other registered hosts' agents are never summed in, matching the titlebar strip's per-window identity claim. That makes the badge stale the moment the window points somewhere else, so the shell **clears it at three seams**: `switchToHost` (before loading the incoming host), `showWelcome`, and the main window's `closed` event (macOS keeps the app alive after `window-all-closed`, so a lingering badge would report a count no window backs). The incoming page re-reports once its own stream is up; there is no shell-side count state to carry across.
...
`version`/`platform` are readable by **every** page, including pages loaded from registered hosts — this is the SPA's shell-detection seam. The four invoker groups are likewise exposed everywhere but privileged by **main-side sender-frame gating**, each against an allowlist (IPC payloads are structurally validated in main — unknown-typed, narrowed — before use):
...
- **IPC hardening**: sender-frame gating on every `welcome:*`, `daemon:*`, `servers:*`, and `badge:set` handler, each against its own allowlist (§ Bridge above) — always in main, never in the preload. Payloads are structurally validated before use: `badge:set` accepts only a non-negative integer, and `fallbackStripCss` hex-validates its color before interpolating it into injected CSS.
...
Verification split: compile, `tsc --noEmit`, node:test (store, window-open policy, local-daemon pure logic, update-check pure logic, strip pure logic, badge pure logic), and vitest (`shell.test.ts`) all run on Linux. Playwright does not cover the Electron shell at all — `isShell()` is false there, so the strip and badge reporter never mount in an e2e run — which is why the pure-module suites plus the compile gates are the automated surface. Hardware-only items, per platform:
...
### The strip color rides the existing `theme-color` meta; no new color API exists
**Decision**: Window-controls-overlay color comes from `webContents.on("did-change-theme-color")`, and the strip's own background comes from the SPA's accent state directly. No SPA→shell IPC channel carries a color.
**Why**: The `theme-color` meta is already accent-aware end-to-end — the provider writes the titlebar blend into it on every accent/theme change, and `index.html`'s pre-paint script echoes it before first paint — so Chromium hands the shell exactly the value it would have asked for, at exactly the moments it changes, for free. A dedicated channel would duplicate that state, add a privileged surface to gate, and introduce a second ordering to reason about against a meta write that happens anyway.
**Rejected**: A `titlebar:color` IPC channel (a second copy of state the meta already broadcasts, plus a gate and an ordering hazard); reading the meta tag from main via `executeJavaScript` (a poll or a race where the event is a push); passing the color through the URL or a query param (nothing to update it on a live accent change).
*Introduced by*: 260731-ofws-shell-accent-titlebar-dock-badge
...
### The badge counts waiting agents only, scoped to the window's own host
**Decision**: The dock/taskbar badge shows the count of windows whose rolled-up `agentState` is `waiting`, summed across everything the connected instance's SSE stream covers — never busy or idle, and never another registered host's agents. It is cleared on host switch, on welcome navigation, and on window close.
**Why**: An OS badge is the most interruptive signal the app owns, so its meaning must be unambiguous: waiting-only makes a non-zero badge always mean "act now", while including busy would make it a workload gauge that is non-zero almost always and therefore ignorable. Scoping to the window's host follows from what the SPA can actually know — it streams one instance — and it keeps the badge's claim identical to the titlebar strip's: this window, this host. Clearing at the three navigation seams is what stops a count from outliving the page that reported it; the incoming page re-reports once its own stream is up, so no shell-side state is carried.
**Rejected**: Counting busy+waiting (a near-permanently-nonzero badge trains the user to ignore it); aggregating every registered host (the SPA has no data for hosts it is not connected to, and it would contradict the strip's per-window identity claim); a taskbar progress bar or a per-route window title (presented alongside and not taken — badge only for now); keeping the old count until the new page reports (a visibly wrong count for the duration of a load).
*Introduced by*: 260731-ofws-shell-accent-titlebar-dock-badge
