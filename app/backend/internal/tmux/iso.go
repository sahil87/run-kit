...
// once the session re-probes as present.
//
...
func EnsureIsoSession(ctx context.Context, server, windowID string) (string, error) {
	...
	// Idempotent reuse: the iso session already exists → return it untouched
	// (a second isolated viewer shares it; no re-link, no modification).
	...
	if _, err := tmuxExecServer(ctx, server, "new-session", "-d", "-s", isoSession, "-c", ServerBirthDir()); err != nil {
		...
		// That race outcome is success once the session provably exists.
		if strings.Contains(err.Error(), "duplicate session") {
			if _, herr := tmuxExecRawServer(ctx, server, "has-session", "-t", ExactSessionTarget(isoSession)); herr == nil {
				return isoSession, nil
			}
		}
