---
description: "Component conventions; modal dialogs and popup surfaces including the spring-loaded marker pad and the GUI off-confirm (with the settings-seam gui.enabled off-interception and the per-viewer gui postures (rk-gui-view/rk-gui-lock, the bare-WM strip dismissal)); clipboard and e2e host-state rules; the Zustand window store, optimistic mutation feedback, and recently-closed mirror."
type: memory
---
...
`app/frontend/src/components/gui-off-dialog.tsx` — the GUI off-confirm (spec `docs/specs/gui.md` § The switch), the `server-dialogs.tsx` danger-confirm grammar on the shared `Dialog` shell (title `Turn the GUI off?`, **Cancel** / **Turn off** danger pair). ONE mount lives at `AppLayout` in `app.tsx`, shared by two open paths through the **`gui-off-context` seam** (`contexts/gui-off-context.tsx` — `open()` self-posting for the palette's `GUI: Turn off`; `request() → Promise<boolean>` verdict-deferring for the settings seam, which must `updateEntryValue` with the same write; `undefined` outside the provider ⇒ callers fall back to the direct POST). (o2sp)
