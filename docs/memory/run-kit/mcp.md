---
type: memory
description: "The rk MCP surface — the `rk mcp` stdio server and the daemon's `/mcp` streamable-HTTP route over `internal/mcp`: the compiled-in policy table (a verb with no row is not a tool), Cobra-introspection schemas with the `Resolve` drift guard, the argv `Executor` (stdin, 45 s cap, `TMUX` stripped), the three-tier JSON result parser, fourteen seeded tools (`send`/`answer`/`await` ride the `--json` envelope receipts), the `OriginPolicy`-guarded SDK transport, the `rk doctor` rows, and the E2E harness."
---
# rk MCP Surface
