...
**Vocabulary split** (`260702-6m46-full-width-topbar-navbar`) — canonical page names are no longer docs-and-conversation ONLY. The two page-like surfaces now surface their canonical name in the page body via the `PageHeading` (`[ cockpit ]`, `[ server cabin · {server} ]` — see § Page Heading). The split is deliberate: **breadcrumbs speak instance names** (server / session / window — *where you are*), **page headings speak canonical page names** (*what the page is*). This supersedes the earlier "canonical names stay chrome-free / docs-only" note wherever it appeared — canonical names stay OUT of the breadcrumb, but ARE the page heading. Terminal and Board carry no page heading (workspace surfaces), so their canonical names remain docs-only.
...
**Sidebar clicks navigate optimistically AND mutate.** `navigateToWindow(windowId)` in `app.tsx` (app.tsx:479-491) fires an optimistic `navigate({ replace: true, to: "/$server/$window", params: { server, window: windowId } })` at click time *and then* calls `selectWindow(server, windowId)` to bring tmux into agreement. The optimistic navigate is what makes the terminal render immediately (including the first click from the `SessionTiles` density view and cross-session clicks the SSE writeback alone can't express); `pendingClickRef` suppresses the writeback's bounce-back until the SSE snapshot confirms the switch (typically sub-500ms with the tmuxctl control-mode subscription; see `architecture.md` § tmux Control-Mode Subscription). Server-side overrides during a click (another agent or hook calling `tmux select-window` concurrently) resolve to whichever mutation tmux processed last — there is no client-side timer or debounce that prefers the click's intent over the external event. On mobile, `navigateToWindow` additionally closes the overlay sidebar after a destination tap.
...
**Layout**: Outer wrapper `flex-1 min-h-0 flex flex-col` with two regions: (1) pinned header row (`shrink-0 px-4 sm:px-6 pt-4 sm:pt-6`) — the shared `<PageHeading page="server cabin" name={server} side="{N} sessions, {M} windows" />` (the Server Cabin's page identity; `260702-6m46` REPLACED the former plain `"{N} sessions, {M} windows"` stats line with this bracketed BBS-tag heading — the stats now sit right-aligned as its `side` text; see § Page Heading); (2) scrollable tile area (`flex-1 min-h-0 overflow-y-auto ...`) whose grid is preceded by a `SESSIONS` uppercase section subheading (`text-xs uppercase tracking-wide text-text-secondary`, the same zone-heading idiom as the Cockpit's HOST HEALTH / BOARDS / TMUX SERVERS / SERVICES, added by `260702-6m46`). Only the tile area scrolls.
...
`PrStatusLine({ win })`, the one-line PR display. Since `260701-70a0` (Dashboard deletion) its only live render site is the Pane panel `pr` row context; the file primarily survives as the colocation home of the StatusDot's exported vocabulary (below). *(Historically it also rendered on the Dashboard window card — that card, and the Dashboard component, are gone.)*
...
`StatusDot({ win }: { win: WindowInfo })` (`app/frontend/src/components/status-dot.tsx`) is the **one** dot reused on all three surfaces — the sidebar window row (leading position), the `SessionTiles` window tile (was the dashboard window card until `260701-70a0` replaced the Dashboard — the call site simply moved into `session-tiles.tsx`), and the pane-panel header. `260615-0hsz` redesigned it from the two-input PR-or-activity model (`260615-yg7f`) into a **three-input lifecycle color journey** that adds the **fab pipeline** as a first-class input. It uses **TWO orthogonal visual channels**: **hue = phase** (where in the lifecycle), **shape = status** (health), with ONE shape vocabulary spanning fab stages AND the PR.
...
> **`PR_DOT_COLOR`/`PR_DOT_LABEL` are GONE.** `260615-yg7f` had the dot render its PR branch via `prDotState` + `PR_DOT_COLOR`/`PR_DOT_LABEL`. `260615-0hsz` deleted both maps (they became zero-call-site once the dot stopped using them — `status-dot.tsx` was their sole consumer). The dot now renders from `PHASE_HUE` + the shape maps. Any earlier claim that the dot uses `PR_DOT_COLOR`/`PR_DOT_LABEL`, or that they "remain live", is FALSE. (`prDotState`/`PrDotState`/`isFailish`/`PR_STATE_COLORS`/`PR_CHECKS_COLORS`/`PR_REVIEW_COLORS`/`PrStatusLine` are UNCHANGED — they serve the Pane-panel `pr` row + its segments, and `prShape` reuses `prDotState`. `PrStatusLine`'s former dashboard-card render site is gone since `260701-70a0`.)
...
**Call sites** — the three surfaces stay bare `<StatusDot win={win} />` (the hover-card lives inside `StatusDot` via the `renderDot` callback): (1) sidebar `WindowRow` — ONE leading dot; (2) the `SessionTiles` window tile — leading dot before the window name (this call site moved out of the deleted `dashboard.tsx` into `session-tiles.tsx` in `260701-70a0`; the tile shows no `{win.activity}` word and no `PrStatusLine`, only the dot + name + fab-stage badge + text preview); (3) pane panel `WindowPanel` `headerRight` — `<StatusDot win={win} />` before `{win.name}`. The dot itself is frontend-only — no backend/API/SSE/tmux change (all inputs already on `WindowInfo` via SSE); no new color tokens (`text-blue-400`/`text-amber-400` are standard Tailwind like the existing `text-yellow-400`; the rest pre-existed). Tests: `status-dot.test.tsx` (three-way precedence + `fabPhase`/`fabShape`/`prShape` mappings + tmux fallback + a11y label composition; since `260616-37ub` the `title`-attribute assertions became `aria-label`/`toBeNull` and a `dotTipContent` describe block was added); `session-tiles.test.tsx` covers the tile's dot render (the old `dashboard.test.tsx` was deleted with the Dashboard); `pr-status-line.test.tsx`'s `prDotState`/`PR_*` tests pass unmodified.
...
BoardPage uses the shared `<Shell>` wrapper with the full-width-topbar desktop grid `grid-template-areas: "topbar topbar" / "sidebar content" / "sidebar bottombar"` (the topbar spans BOTH columns; the sidebar occupies rows 2–3 — the topology `260702-6m46-full-width-topbar-navbar` established for every `<Shell>` consumer; collapses to single-column on `< 640px` with the sidebar overlay). Children are placed into named grid areas:
...
- **TopBar** (`gridArea: "topbar"`) — `<TopBar mode="board" boardName={name} paneCount={entries.length} serverCount={uniqueServers}>`. Board mode renders brand + hamburger + `›` + `BoardModeBreadcrumb` (`Board ▸ {name} ▾` — the board switcher via the shared `BreadcrumbDropdown`, listing `← Sessions` + other boards with `(current)` on the active one) followed by inline-info `{N} pane[s] · {M} server[s] · ⌘[⌘] cycle` (singular/plural correct, `text-xs text-text-secondary`, `hidden sm:inline`). The right section's route-agnostic chrome (FixedWidth / Notification / Theme) matches other modes; `currentWindow` is null so split/close don't render, and the connection dot is hidden (board mode)
...
The AppShell palette mount carries `Switch to <name>` + `Pin Current Window` + `Unpin Current Window`. The BoardPage's own palette mount (`boardRouteActions` useMemo in `board-page.tsx`) carries `Switch to <name>` + `Leave Board View` + `Cycle Pane Focus →/←` — board-route-only entries that AppShell's palette can't surface because the board route does not render AppShell — **plus** the globally-meaningful actions duplicated in because the board mounts its own palette: the terminal-font trio (Increase / Decrease / Reset terminal font — the board's panes are live terminals, § Terminal Font Size) followed by "View: Refresh Page". The returned array composes as `[...switchEntries, ...conditional, ...fontEntries, refreshEntry]`, so **`refreshEntry` sits last, immediately after the terminal-font trio** (`260703-p29w-topbar-refresh-button`). The board is the intake's core degraded-relay recovery scenario — N live relay WebSockets, and no top-bar RefreshButton since `currentWindow` is `null` on a board route — so this palette entry is the board's only reload affordance.
...
The `<Shell>` wrapper (`app/frontend/src/components/shell/shell.tsx`) is the shared layout for `AppShell` and `BoardPage` (the Cockpit `/` uses a plain `flex-col h-screen` instead — no Shell). Since `260702-6m46-full-width-topbar-navbar` the **desktop** grid is a full-width topbar:

```
gridTemplateAreas: "topbar topbar" / "sidebar content" / "sidebar bottombar"
```

The topbar spans BOTH columns (the app's primary chrome, reading as full-width app furniture); the sidebar occupies rows 2–3 (content + bottombar) only, below the topbar. `gridTemplateColumns` stays `${sidebarWidth}px 1fr` when `sidebarOpen` else `0 1fr` (a `150ms ease-out` collapse transition, zero-width column when closed — no rail); `gridTemplateRows` stays `auto 1fr auto`. Before `260702-6m46` the topbar shared row 1 with the sidebar (`"sidebar topbar"`); the grid edit was one line, and the ASCII topology comment in `shell.tsx` was rewritten to match.

The **mobile** (`< 640px`) branch is unchanged: single column, `gridTemplateAreas: "topbar" "content" "bottombar"`, with the sidebar rendered as an `absolute` overlay spanning `gridRow: "2 / 4"` (below the topbar so the hamburger stays a visible close affordance — see § Sidebar Mobile). Children place themselves via `style={{ gridArea: … }}`.

## Chrome (Top Bar)

The full-width top bar is the app's primary chrome and doubles as the navbar. `app.tsx` and `board-page.tsx` render `<TopBar>` into the Shell grid's `topbar` area (which spans BOTH columns — see § Shell Grid Layout); `server-list-page.tsx` (Cockpit `/`) pins it above its own scroll container (no Shell). `TopBar` (`app/frontend/src/components/top-bar.tsx`) accepts a `mode: "terminal" | "board" | "root" | "cockpit"` prop (default `"terminal"`) selecting the left-nav content; the `260702-6m46-full-width-topbar-navbar` change added `"cockpit"` and turned the left nav into a breadcrumb bar. The right-section control cluster is nearly mode-invariant (only the connection dot and terminal-font/split/close controls gate on mode).

**Layout** (`border-b-[3px] border-border` — a 3px structural seam, see § Border-Width System): a single line, `flex justify-between`. LEFT is a `<nav aria-label="Breadcrumb">`; RIGHT is the control cluster. No Line 2.
...
**Cockpit** (`/`, `mode="cockpit"`): brand crumb ONLY — no hamburger, no crumbs, no connection dot (the Cockpit has no per-server SSE stream), no terminal-font control, no split/close buttons. Keeps the three route-agnostic controls (FixedWidth / Notification / Theme). Session/server-dependent props are passed empty (`sessions=[]`, `currentSession=null`, `currentWindow=null`, `sessionName=""`, `server=""`, no-op callbacks) — the same tolerant-empty shape board mode uses. The Cockpit's page identity is instead carried by the `PageHeading` in the page body (see § Page Heading).

**Server Cabin** (`/$server`, no `$window`, `mode="root"`): brand + hamburger + `›` + the server name as the current-page leaf — a non-link `<span aria-current="page" class="text-text-primary font-medium truncate">`. This REPLACED the literal "Dashboard" label (deleted by `260702-6m46`; "Dashboard" no longer appears in any mode). As the leaf it carries no link/dropdown affordance and stays visible on mobile.

**Terminal** (`/$server/$window`, `mode="terminal"`): brand + hamburger + `›` + server link + `›` + session dropdown + `›` + window dropdown. The server crumb here is a plain `<a href="/$server">` (a link back to the Server Cabin, `LINK_CRUMB_CLASS` chip, `truncate max-w-[16ch]`) — NOT a dropdown (server switching stays in the command palette, `Server: Switch to …`). Session and window crumbs keep their existing `BreadcrumbDropdown` behavior unchanged (session switch jumps to the session's first window; `+ New Session` / `+ New Window` actions; session `max-w-[16ch] truncate` (matches the server crumb); the session name is snapshot-derived from the active `@N`, not a URL segment, since `260529-jad6`). No type-prefix words ("SERVER"/"Cabin"/"session") — crumbs are instance names only; position conveys the level. The level IS named on hover via native `title` tooltips in the canonical vocabulary: brand `title="Cockpit"`, server crumb `title="Server Cabin"` (both link and leaf variants), session/window dropdown triggers `title="Session"`/`title="Window"` (via BreadcrumbDropdown's optional `title` prop).

**Board** (`/board/$name`, `mode="board"`): brand + hamburger + `›` + the existing `BoardModeBreadcrumb` (`Board ▸ {name} ▾    {N} pane[s] · {M} server[s] · ⌘[⌘] cycle` — the board switcher dropdown + inline-info span, `hidden sm:inline` below 640px; internals unchanged by `260702-6m46`). Board mode hides the connection dot (per-server state, board spans servers).

**Mobile (`< sm`)** — the brand collapses to the bare icon and intermediate crumbs (server link + session, plus their `›` separators, wrapped in `hidden sm:flex` spans) are hidden, leaving brand icon + the leaf crumb (window dropdown on Terminal, server-name leaf on Server Cabin, board name on Board). The topbar stays a single line at 375px (the documented invariant in `fab/project/context.md`).

**Right cluster** (`flex items-center gap-3`, mode-invariant order): terminal-only controls (split-vertical, split-horizontal, close-pane, refresh-page — rendered only when `currentWindow` exists) → terminal-font `Aa` control (gated `mode !== "root" && mode !== "cockpit"` — neither has a terminal to size; board keeps it because its panes are terminals) → the three route-agnostic controls FixedWidth → Notification → Theme → the connection dot as the **right-most** element. Each control sits in a `hidden sm:flex` span (mobile hides the cluster). The connection dot (`role="status"`, green/gray) renders only when `mode !== "board" && mode !== "cockpit"` (i.e. terminal + root). With the brand anchor gone the dot is now the right edge — `260702-6m46` rewrote the icon-ordering comment to drop the old "pinned to the Run Kit anchor" rationale.
...
Breadcrumb crumbs advertise their interactivity WITHOUT hover — a two-affordance vocabulary where the absence of an affordance is itself meaningful (it marks the current-page leaf). The three cases:
...
- **Dropdown triggers** (session, window, board) — carry the persistent `▾` caret inside `BreadcrumbDropdown` (see § Breadcrumb Dropdowns). Signals "opens a switcher", distinct from "navigates".
- **Current-page leaf** (the `aria-current="page"` server-name leaf on Server Cabin) — carries NEITHER. The missing affordance is the "you are here" signal.
...
### Page Heading (`page-heading.tsx`, `260702-6m46`)

A shared `PageHeading({ page, name?, side?, className? })` renders a one-line **bracketed BBS-menu-tag** heading — `[ page · name ]────… side` — on exactly the two **page-like** surfaces; workspace surfaces (Terminal, Board) carry NO page heading (their vertical space belongs to the terminal). This establishes the **vocabulary split** that supersedes the earlier "canonical names stay docs-only" stance (see § Canonical page names — Vocabulary Split note): the **breadcrumb speaks instance names** (where you are), **page headings speak canonical page names** (what the page is).

- **Structure**: the bracket group holds the canonical page word (lowercase) and, when the page is about an instance, the instance `name` after a `·`. The page word renders **secondary** (`text-text-secondary`) when a `name` follows (the name is the subject, `text-text-primary font-medium`) and **primary** standalone. A `flex-1 border-t border-border` fills the middle as the rule (a CSS border — responsive, no literal `─` glyphs to overflow at 375px); optional `side` text sits right-aligned after it.
- **Accessibility**: brackets, the `·` separator, and the rule are `aria-hidden`; the `<h1>` is the accessible structure. The literal `" "` text nodes between page word and name keep the computed accessible name word-separated (e.g. "server cabin testServer") — flex drops whitespace-only items so they never render. Instance names are never case-transformed (server names are case-sensitive identifiers).
- **Cockpit** (`/`): `<PageHeading page="cockpit" />` — no instance, no side-text — above the HOST HEALTH section (`server-list-page.tsx`). This revised the intake's "no Cockpit heading" decision (assumption #11, revised twice by user decision during the visual pass).
- **Server Cabin** (`/$server`): `<PageHeading page="server cabin" name={server} side={"{N} sessions, {M} windows"} />` as the pinned header row (`session-tiles.tsx`), REPLACING the plain stats line. The stats are right-aligned after the rule.
...
- `status-panel.tsx` — `WindowPanel` (exported as both `WindowPanel` and deprecated `StatusPanel`); wraps pane metadata rows (tmx, cwd, git, run, agt, fab, pr) in a `CollapsiblePanel` with copyable row interactions (via the file-private `CopyableRow`). The `pr` row (added by `260610-obky-pr-status-to-pane-panel`) is the primary live-PR-status surface, gated change-bound (`getPrSegments` colored segments); its interaction is **open-first** since `260703-41ks-pr-row-open-first-action` (the one row that isn't click-to-copy) — with a PR URL the row body is a real anchor (file-private `PrLinkRow`) opening the PR in a new tab (always-visible inline `↗`), and copy is role-swapped to a hover-revealed right-side icon (reusing the window-row icon-cluster gating); without a URL it stays a plain `CopyableRow` copying the segment text. See § PR Status. The panel's `headerRight` now leads with a unified `<StatusDot win={win} />` before `{win.name}` (`260615-yg7f-unified-status-dot`) — an at-a-glance status summary that survives panel collapse (`CollapsiblePanel` keeps `headerRight` visible when collapsed); see § Status Dot
...
**CollapsiblePanel** (`app/frontend/src/components/sidebar/collapsible-panel.tsx`) — reusable wrapper used by both Window and Host panels. Props: `title` (string), `storageKey` (string for localStorage persistence), `defaultOpen` (boolean, default `true`), `children` (ReactNode). Header is always visible: title text + chevron (`&#x25B8;` U+25B8) that rotates 90 degrees on toggle via CSS `transform: rotate()` with `transition-transform duration-150`. Content area uses `max-height` transition (`duration-150 ease-in-out`) for smooth expand/collapse. `overflow: hidden` during transition, `visible` when fully expanded (accessibility). Collapse state persisted to `localStorage[storageKey]` on every toggle. Each panel has `border-t-[3px] border-border first:border-t-0` (a 3px structural seam — see § Border-Width System — so panels partition cleanly; the resizable drag handle is transparent by default, letting the next panel's 3px top border be the sole visible inter-panel line).
...
Since `260701-vshd-cockpit-host-console-home`, `ServerListPage` (`app/frontend/src/components/server-list-page.tsx`, route `/`) renders a **HOST HEALTH** zone ABOVE the tmux-server tile grid, inside the scroll container (`div.flex-1.min-h-0.overflow-y-auto`, which since `260702-6m46` carries `pt-6` so the gap below the pinned TopBar matches the `mb-6` inter-section rhythm). This makes `/` — the only route that is about the BOX, not a session — a machine console. Since `260702-6m46-full-width-topbar-navbar` added the BOARDS zone, the zones render in top-to-bottom order **HOST HEALTH → BOARDS → TMUX SERVERS → SERVICES** (each an uppercase-labelled `<section>`; the server-tile grid gained a "Tmux Servers" heading + inline server count, and SERVICES moved to last, after the tiles — `260701-5d0z-service-tiles-host-console`; see § BOARDS Zone, § SERVICES Zone on `/`), all beneath the `[ cockpit ]` `PageHeading`. Read-only awareness view; nothing here mutates host state (Constitution IV).

- The zone (`<section aria-label="Host health">`) shows an uppercase "Host Health" heading + the live `hostname` (from `useHostMetrics()`) in a `gap-3` heading row (the shared idiom — `260702-6m46` demoted the hostname to secondary mono side-text, `text-text-secondary font-mono`), then the shared `HostMetrics` component (CPU sparkline+current, memory gauge, load 1/5/15 normalized, disk used/total, uptime).
...
- **Heading row**: the shared idiom — `gap-3` between the uppercase "Boards" heading and its side-text; side-text shows `loading…` before the first fetch (`isLoading`), else `{N} board(s)` (`text-xs text-text-secondary font-mono`).
- **Tiles**: one `<button>` per board (same tile idiom as the server tiles — `bg-bg-card border border-border rounded p-4 hover:border-text-secondary min-h-[60px]`) showing the board name + `{N} pin(s)`; clicking navigates to `/board/$name`.
...
- **1px hairlines** (`border` / `border-t` / `border-b`) — the default: card borders, tile borders, in-panel dividers, dropdown separators, the `PageHeading` rule.
- **3px structural seams** (`border-t-[3px]` / `border-b-[3px]` / `w-[3px]`) — the major chrome boundaries that partition the app frame: the topbar's bottom edge (`top-bar.tsx`, was `border-b-2`), both bottombars (`app.tsx` + `board-page.tsx`, each was `border-t`), the sidebar's inter-panel separators + Sessions-section top (`collapsible-panel.tsx` + `sidebar/index.tsx` — these already used 3px, and the polish aligned the other seams to match them), and the sidebar resize handle bar (`app.tsx`, was `w-[5px]`; see § Sidebar → Desktop). The intent: seams read as deliberate structure, hairlines as content detail.
