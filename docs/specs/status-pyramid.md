...
| Hover card (row flyout) | full detail | hue-word + liveness-word label + flags, the `fab`/`pr`/`opr` registers (`out`/`agt` omitted — the row already carries them), PR link, docs link |
