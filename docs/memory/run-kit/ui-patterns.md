...
**Window cards** (inside expanded session): Each window card is a button (`bg-bg-primary border border-border rounded`) that selects the window (`@N`) on click; the URL follows to `/$server/$window` on the next SSE snapshot. Shows:
