---
description: "Per-window focus memory (tty/compose/code) and the code-server steal guard: real-focus-only recording asymmetry, the desktop restore router with its tty first-visit default, the guard arm/disarm lifecycle, the in-frame focusin grab detector, and the focus-seam consumers — the focus-hop chord (⌃`) recording only on the tty hop, and the web tile's IframeWindow reporting through the same seam shape without recording."
type: memory
---
