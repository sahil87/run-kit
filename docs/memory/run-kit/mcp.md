...
- `new_window` (`tab new`): `server`, `session` (`^=.+$`), `cwd`, `name`, `layout` — no `--ready`, no `-- CMD`; the window is born idle. The verb's `{session, session_rung, window_id, pane_id}` document rides the envelope inside `result` like the sibling rows; the always-present `session_rung` says why the landing session was chosen, and over MCP it always reads `server` — the executor strips `TMUX`/`TMUX_PANE`, so there is no caller session and the role-aware ladder is unreachable (xga4). The row's description override states that shape and the infra-caller rule.
