...
**Not registered** (transient, edge-overlapping — the clip rule's class): `Tip`, the row flyout card, the marker pad, the pin popover, the inline `SwatchPopover` mounts, the embedded `ThemePickerList`, the compose-history flyout, the status-bar and top-bar menus, the compose strip (not a dialog), and the inline cron sheet (it lives inside the registered quake drawer). Toasts are neither: on desktop they sit over the status bar, off the stage (§ Error toast system).

**Consumer seam**: readers call `subscribe` + `isModalOpen()` (or `useSyncExternalStore(subscribe, isModalOpen)` in React). The subscriber is the web tile's native engine (`components/web-frame-native.tsx` — [lenses-and-layout](/run-kit/ui/lenses-and-layout.md) § Web Tile): it reads `modalOpen` via `useSyncExternalStore(subscribe, isModalOpen, () => false)` and hides its guest (`web:visible {false}`) while `isModalOpen()`, re-showing — after a fresh bounds measure — on release; `transient` stays counted with no consumer. Covered by `lib/overlay-presence.test.ts`, `hooks/use-occludes.test.tsx`, and one acquire/release assertion in each registering component's colocated test. (q2xk)
