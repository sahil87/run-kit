...
- **Install gating**: the PATH block is written only when the shim is in place
  ...
  **uninstall** the two pieces are independent: the block is stripped even when the
  ...
  — it gates on its own per-machine pointer symlink (`~/.local/share/rk/bin/run-kit`),
  the same relocatable-block / per-machine-artifact split the PATH block and shim
  use. (bbv1) (r7v8)
