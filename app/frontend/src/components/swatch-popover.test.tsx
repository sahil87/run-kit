...
describe("SwatchPopover", () => {
  ...
  describe("composite preview", () => {
    ...
    it("renders the row's resting look: tint, row name, and the combo caption (no marker stripe)", () => {
      ...
      // The marker is not this picker's axis — the preview draws no stripe and
      // no hazard texture.
  ...
  // ── Banded structure: [ color ] scroll strip + [ flair ] 2-row strip, each
  //    header carrying its − clear cell (no marker band — the marker axis has
  //    its own pad). ──
  ...
  describe("keyboard navigation (plain-grid bands)", () => {
    ...
    it("vertical moves preserve the column as a goal column, clamped to the target row's extent", () => {
      ...
      // single-cell header − row (goal column) and clamps to 7 (noon) on the
      // 8-cell flair row 1.
      arrow("ArrowDown", 4);
      enter();
