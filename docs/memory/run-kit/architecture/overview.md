...
- Web-tile tunnel: `GET /ws/tunnel?target=host:port` carries byte tunnels for the desktop shell's native web engine over a WebSocket (an ordinary `GET` upgrade — no §IX exception), with **no destination restriction** — anyone who can reach rk already has a shell on the host through the terminal relay (no auth; Tailnet-only / SSH-tunnel-only by deployment), so a destination policy would add no protection. WebSocket Origin policy (browsers apply no CORS to WebSocket handshakes, so each upgrader enforces its own gate): the tunnel upgrader admits ONLY requests carrying no `Origin` and no `Sec-Fetch-Site` header — its sole client is the desktop's Electron main process and every browser WebSocket carries an unsuppressable `Origin`, so browser pages are rejected cross-site, same-site, and DNS-rebinding alike; the shared upgrader (`/ws/state`, `/ws/terminals`, `/ws/gui/{id}`) enforces a Fetch-Metadata-first same-origin policy (`Sec-Fetch-Site: same-origin` allowed, any other present value rejected; header absent → a missing `Origin` allowed, else the `Origin` host must match the first `X-Forwarded-Host`/`Host`, scheme-insensitive, default ports normalized). Known limitations: DNS rebinding is not stopped by the shared upgrader's Host-match rule (the tunnel is immune via its no-Origin rule), and CORS stays allow-all (`AllowedOrigins: ["*"]`) — the WebSocket tightening is defense-in-depth for the socket surface; tightening CORS is a follow-up. The tunnel dials with a `net.Dialer` timeout, spawns no subprocess (§I), and holds no state beyond live connections (§II) — see [api-and-sockets](/run-kit/api-and-sockets.md) § Tunnel and [desktop-shell](/run-kit/desktop-shell.md) § Design Decisions → No destination policy on the tunnel
