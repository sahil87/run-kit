...
// palette action all open the shared FindBar over the primary tty tile;
// `@xterm/addon-search` searches the live xterm client buffer with
// decorations (buffer highlights + overview-ruler ticks); Enter/⇧Enter
// navigate with wrap; Escape closes and clears; the terminal-native Aa/`.*`
// toggles and the muted buffer-scope hint behave per contract.
//
// Shared setup: `beforeAll` creates one dedicated session `e2e-ttyfind-<ts>`
// (80×24) on the isolated e2e tmux socket whose single window prints exactly
// three `FAIL` lines followed by `sleep 300` — a fixed, countable
// client-buffer payload; `afterAll` kills the session (best-effort).
// `beforeEach` sets a wide desktop viewport (1440×800). `gotoTtyWindow(id)`
// deep-links the window's terminal route (default `single:tty` — ONE tile)
// and waits for the `.xterm-screen` element. `awaitPaneOutput(id)` polls the
// registered terminal's (`window.__rkTerminals`) active buffer until all
// three `FAIL` lines have streamed in — no fixed sleeps. `focusTerminal`
// clicks the xterm area and focuses the helper textarea so keydowns arrive
// through the genuine input path. The per-test timeout is 30s (60s on CI) —
// the snapshot poll + route load + buffer poll exceed the 10s default on a
// contended runner.
...
test.describe("Terminal tile — find in buffer (260819-zqf9)", () => {
  ...
  /**
   * Proves: the full find flow on a tty tile — the shifted-tier chord the
   * Linux rig resolves (⇧Ctrl+F; ⌘F on mac) opens the bar under terminal
   * focus via the terminal seam's refusal rule, the counter tracks the
   * addon's `onDidChangeResults`, navigation wraps, decorations render (and
   * the overview ruler exists at all — the constructor `overviewRuler`
   * option), the buffer-scope note appears once a search has run, and
   * Escape closes the bar, clears every decoration, and returns focus to
   * the pane.
   *
   * Steps:
   * 1. Resolve the window, open its terminal route, wait for the pane
   *    payload.
   ...
   * 4. Assert find decorations exist in the DOM decoration layer (a count
   *    poll — the elements size with the render loop) and the
   *    `.xterm-decoration-overview-ruler` element renders.
   * 5. Assert the `Search scope` note reads `… since attach`.
   * 6. Press Enter three times: counter reads `2/3`, `3/3`, then wraps to
   *    `1/3`; press `Shift+Enter`: reads `3/3`.
   * 7. Press Escape: the bar is gone, no find decorations remain, and the
   *    xterm helper textarea holds focus again.
   */
  ...
  /**
   * Proves: the pointer entry point — the header button opens and closes
   * the bar and carries the active vocabulary (`aria-pressed` +
   * accent-green) while open.
   *
   * Steps:
   * 1. Open the terminal route; wait for the pane payload.
   * 2. Assert the ⌕ button starts `aria-pressed="false"`; click it; assert
   *    the bar is visible, the button is `aria-pressed="true"` and
   *    accent-green.
  ...
  /**
   * Proves: the palette discovery surface — the action exists while the
   * rendered layout includes a tty tile and opens the bar through the same
  ...
  /**
   * Proves: the edge contracts — a no-match query floors the counter at
   * `0/0` with navigation a no-op, and a malformed pattern under the `.*`
   * regex toggle is caught (addon throw → `false`) without breaking the
   ...
   * 3. Click the `Match regex` toggle; fill `([`; assert the bar stays
   *    visible and the counter reads `0/0`.
