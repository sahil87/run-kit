...
func TestSetServerFlair_persists(t *testing.T) {
	...
	got := settings.GetServerFlair("dev")
	if got == nil || *got != "nyan" {
		t.Errorf("persisted flair = %v, want \"nyan\"", got)
	}
}
...
func TestSetServerFlair_rejectsInvalid(t *testing.T) {
	for _, bad := range []string{`{"server":"dev","flair":"pikachu"}`, `{"server":"dev","flair":"Nyan"}`, `{"server":"dev","flair":"one-piece"}`, `{"server":"dev","flair":" nyan "}`, `{"server":"dev","flair":"bogus"}`} {
		...
		if got := settings.GetServerFlair("dev"); got != nil {
			t.Errorf("body %s: invalid value persisted as %q, want nil", bad, *got)
		}
	}
}

// Both a JSON null and an empty string clear the flair (the "empty = unset"
// contract shared with the window/session @rk_flair options).
func TestSetServerFlair_clears(t *testing.T) {
	isolateSettings(t)
	router := newTestRouter(&mockSessionFetcher{}, &mockTmuxOps{})

	for _, clearBody := range []string{`{"server":"dev","flair":null}`, `{"server":"dev","flair":""}`} {
		flair := "naruto"
		if err := settings.SetServerFlair("dev", &flair); err != nil {
			t.Fatalf("seed: %v", err)
		}
		rec := postJSON(t, router, "/api/settings/server-flair", clearBody)
		if rec.Code != http.StatusOK {
			t.Fatalf("body %s: status = %d, want %d; body=%s", clearBody, rec.Code, http.StatusOK, rec.Body.String())
		}
		if got := settings.GetServerFlair("dev"); got != nil {
			t.Errorf("body %s: flair after clear = %q, want nil", clearBody, *got)
		}
	}
}
...
// GET with no server param returns the full map; GET with ?server= returns the
// single flair (null when unset). Mirrors the server-color GET shapes.
func TestGetServerFlair_fullMapAndSingle(t *testing.T) {
	isolateSettings(t)
	router := newTestRouter(&mockSessionFetcher{}, &mockTmuxOps{})

	nyan, naruto := "nyan", "naruto"
	if err := settings.SetServerFlair("default", &nyan); err != nil {
		t.Fatalf("seed: %v", err)
	}
	if err := settings.SetServerFlair("dev", &naruto); err != nil {
		t.Fatalf("seed: %v", err)
	}
	...
	if full.Flairs["default"] != "nyan" || full.Flairs["dev"] != "naruto" {
		t.Errorf("flairs = %v, want {default: nyan, dev: naruto}", full.Flairs)
	}

	if got := getScalarSettingViaAPI(t, router, "/api/settings/server-flair?server=dev", "flair"); got == nil || *got != "naruto" {
		t.Errorf("GET single = %v, want \"naruto\"", got)
	}
	if got := getScalarSettingViaAPI(t, router, "/api/settings/server-flair?server=other", "flair"); got != nil {
		t.Errorf("GET unset = %q, want null", *got)
	}
}
