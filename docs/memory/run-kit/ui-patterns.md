...
- **Tiles**: each `Service` (from `@/types` — `{ port, process?, pid? }`; v1 is port-only, `process`/`pid` reserved/absent) renders a row keyed by `svc.port` showing `:{port}` (mono) + the optional `process` name. Empty state: a graceful `No services` affordance when the list is empty (pre-first-tick, or a non-Linux zero-valued host — the collector returns `[]` off Linux).
