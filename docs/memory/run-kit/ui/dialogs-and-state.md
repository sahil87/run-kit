---
description: "Component conventions; modal dialogs and popup surfaces including the spring-loaded marker pad and the GUI off-/restart-confirms on one shared shell (with the settings-seam gui.enabled off-interception and the per-viewer gui postures (rk-gui-view/rk-gui-lock, the bare-WM strip dismissal)); clipboard and e2e host-state rules; the Zustand window store, optimistic mutation feedback, and recently-closed mirror."
type: memory
---
...
**Per-viewer gui postures** (`lib/gui-posture.ts`, validated reads / try/catch-noop writes — the untrusted-localStorage discipline; three keys): `rk-gui-view` (`"fit"` | `"1:1"`, absent = `"fit"` — the canvas view mode), `rk-gui-lock` (`"1"` = locked, absent = unlocked — a locked viewer never drives SetDesktopSize; viewer-local by design, a second fine-pointer viewer can still resize — the loop-safe host-wide answer is the host pin `rk gui lock`, streamed as the entry's `locked` and honored by every viewer's tile, [gui](/run-kit/gui.md) § Agent verbs), and `runkit-gui-wm-strip-dismissed` (`"1"` = dismissed, absent = shown — the bare-WM strip's dismissal, cleared whenever the stream's `wm` turns non-empty so a later bare state shows the strip again; a `reachable` flip never clears it). For the view/lock pair `app.tsx` owns the state, seeded from these keys, written by the palette's `GUI: Fit`/`1:1` and `GUI: Lock/Unlock resolution` destination-only pairs ([keyboard-and-palette](/run-kit/ui/keyboard-and-palette.md) § The `GUI:` palette family); the tile consumes them as props (§ [lenses-and-layout](/run-kit/ui/lenses-and-layout.md) § GUI Surface). The strip-dismissal key is owned by the gui tile itself — read on mount, written by the strip's × and by the `wm`-non-empty clearing effect.
