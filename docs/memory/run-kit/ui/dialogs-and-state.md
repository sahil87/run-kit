---
description: "Component conventions; modal dialogs and popup surfaces including the spring-loaded marker pad and the GUI off-/restart-confirms on one shared shell (with the settings-seam gui.enabled off-interception and the per-viewer gui postures (rk-gui-zoom/rk-gui-pointer/rk-gui-lock/rk-gui-hidpi/rk-gui-keybar, the bare-WM strip dismissal)); clipboard and e2e host-state rules; the Zustand window store, optimistic mutation feedback, and recently-closed mirror."
type: memory
---
...
**Per-viewer gui postures** (`lib/gui-posture.ts`, validated reads / try/catch-noop writes — the untrusted-localStorage discipline; six keys): `rk-gui-zoom` (`"fit"` | a decimal percentage from `GUI_ZOOM_STEPS` (50/75/100/125/150/200), absent or out-of-table = `"fit"` — the canvas zoom; a one-time migration reads a legacy `rk-gui-view: "1:1"` as `100` when `rk-gui-zoom` is absent, and `writeGuiZoom` removes the legacy key), `rk-gui-pointer` (`"touch"` | `"trackpad"` — the coarse-pointer input mode; absent/invalid resolves the pointer-class default: `trackpad` on coarse, `touch` on fine), `rk-gui-lock` (`"1"` = locked, absent = unlocked — a locked viewer never drives SetDesktopSize; viewer-local by design, a second fine-pointer viewer can still resize — the loop-safe host-wide answer is the host pin `rk gui lock`, streamed as the entry's `locked` and honored by every viewer's tile, [gui](/run-kit/gui.md) § Agent verbs), `rk-gui-hidpi` (`"1"` = on, absent/other = off — the HiDPI rendering switch: the percentage-zoom host CSS size divides by `devicePixelRatio`, so a 100% zoom maps one framebuffer pixel to one device pixel; client-side rendering only, never a server-facing size), `rk-gui-keybar` (`"0"` = hidden, absent/other = shown — the key bar's visibility; the default keeps the bar rendering), and `runkit-gui-wm-strip-dismissed` (`"1"` = dismissed, absent = shown — the bare-WM strip's dismissal, cleared whenever the stream's `wm` turns non-empty so a later bare state shows the strip again; a `reachable` flip never clears it). For the zoom/pointer/lock/hidpi/keybar five `app.tsx` owns the state, seeded from these keys (the pointer mode seeded with the coarse-pointer flag), written by the palette's destination-only rows (`GUI: Zoom …`/`GUI: 1:1`, `GUI: Pointer → …`, `GUI: Lock/Unlock resolution`, `GUI: HiDPI on/off`, `GUI: Hide/Show key bar`), by the tile's zoom chords/Ctrl+wheel/pinch, and by the toolbar pill's chips ([keyboard-and-palette](/run-kit/ui/keyboard-and-palette.md) § The `GUI:` palette family); the tile consumes them as props (§ [lenses-and-layout](/run-kit/ui/lenses-and-layout.md) § GUI Surface). The strip-dismissal key is owned by the gui tile itself — read on mount, written by the strip's × and by the `wm`-non-empty clearing effect. (0aur) (t2lv)
...
| **Appearance** | Theme pair, instance accent color (**This host**); terminal font size, operator console opacity (**This device**) |
...
- **Operator console opacity** (`ConsoleOpacityControl`, under This device, sublabel "Desktop console drawer background; 100% turns off the blur") — the desktop console drawer's glass background ([ui/operator-console](/run-kit/ui/operator-console.md) § Glass background): a range slider (`min`/`max` = the 0.5–1.0 clamp, step 0.05) with an `aria-live` percent readout, backed by the per-viewer localStorage key `runkit-operator-console-opacity` through the console's pub/sub store (`useConsoleOpacity`) — NOT an `internal/settings` registry key and NO `/api/settings` request (the registry is per-instance daemon config; glass is per-eye). An open console reflects changes live via the store's same-tab notify; 100% disables the backdrop blur entirely.
