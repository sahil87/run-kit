...
- **"Open in window" action** (`handleOpenInWindow`): opens the port's UI in an `@rk_type=iframe` tmux window via the existing `/proxy/{port}/` reverse proxy (`createWindow(server, session, ":{port}", undefined, "iframe", "/proxy/{port}/")` — the `rkType`/`rkUrl` args become the `@rk_type`/`@rk_url` window options), then navigates to `/$server` (the new window surfaces via SSE). Since `/` is server-less, it resolves a **target (server, session)**: `servers[0]` + that server's first session from the SSE cache (`sessionsByServer`), else an authoritative `getSessions()` fetch (the per-server stream is usually not attached yet on a fresh `/` load, so the cache can be empty even when sessions exist — fetching avoids a duplicate-name 500 on `new-session`), else an instant `"services"` session created on the fly (`createSession`). An `openingRef` guard debounces a double-click into a single create flow.
