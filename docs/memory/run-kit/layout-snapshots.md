---
type: memory
description: "Per-server tmux layout snapshots (`internal/snapshot`): the sessions/windows/panes + rk-options capture set (incl. `_rk-operator`/`@rk_win_role`), the `$XDG_STATE_HOME/run-kit/snapshots` store (atomic latest, 10-entry history, content-dedup, zero-session guard, `.died-{ts}` tombstones, legacy-dir move + breadcrumb), the Snapshotter cadence + `@rk_srv_ephemeral` opt-out, the restore engine + `rk mux snapshot` CLI (no relaunch; restore stamps `@rk_srv_managed`), and the recovery reader."
---
