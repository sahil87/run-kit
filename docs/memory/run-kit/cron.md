...
- **if_absent dispositions**: for each `EvalResult.Absent` fire, `Tick` applies the entry's `if_absent` policy per the disposition table:
  ...
  |---|---|---|
  | role / session | yes | run the argv via `RunRespawn` under a `DefaultRespawnTimeout` (90s) derived context, `os.UserHomeDir()` as the exec's cwd (the daemon has no project cwd — the operator window's own directory is the launcher's derivation, § Caller-Supplied Respawn), `{server}` substituted with the fire's stamped server name (`RespawnArgv`) → `respawned` on exit 0 (`respawned (kickoff undelivered: <reason>)` when the captured output tail carries the launcher's `kickoff: undelivered reason=<r>` stderr line — `respawnSuccessOutcome`, still `respawned`-prefixed so orphan GC and rate-cap classification hold) / `respawn-failed: <detail>` otherwise (error + ≤200-byte output tail). NO delivery that tick — the payload lands on the next resolved fire |
...
How to bring a target back is the entry creator's knowledge, not the clock's: with `if_absent: respawn` and a non-empty `respawn: [argv...]`, the tick runs the argv as an argument slice via `exec.CommandContext` — NEVER a shell string (Constitution I) — through the `Deps.RunRespawn` exec seam (`RunRespawnFunc func(ctx, argv, dir) ([]byte, error)`; nil selects the production `runRespawnExec`, which sets `cmd.Dir` and captures combined output). `RespawnArgv(e, server)` is the pure render step: the exact substring `{server}` in ANY element is replaced with the fire's stamped server name (`strings.ReplaceAll` on a copy — the entry is never mutated), so the file reads plainly (`respawn: ["rk", "operator", "-L", "{server}"]`). Runtime: `DefaultRespawnTimeout` (90s named constant — it must exceed `rk operator -L`'s server-mode kickoff delivery bound of `operatorServerDeliverDeadline` 60s + `operatorCmdTimeout` 10s plus agent boot, the `cronSessionSpawnTimeout` precedent), working directory the user's home (the daemon has no project cwd), environment the daemon's own (already TMUX/TMUX_PANE-scrubbed by `internal/tmux`'s init). Outcomes: exit 0 within the timeout ⇒ `respawned` — rendered `respawned (kickoff undelivered: <reason>)` by `respawnSuccessOutcome` when the combined output carries the launcher's `kickoff: undelivered reason=<r>` stderr line (the ≤200-byte tail folds it in for free), still `respawned`-prefixed for orphan/rate classification; non-zero exit, timeout, or exec error ⇒ `respawn-failed: <detail>` with the error and a ≤200-byte output tail folded in (`respawnDetail`, `…`-prefixed when truncated). **After a successful respawn there is NO delivery that tick** — a fresh session has no tick convention in context, so the payload lands on the next resolved fire; both outcomes append one log line, advance the anchor, and count toward the absent-fire rate cap (entry-ID keyed) as before. The respawn command owns its own idempotency (no defensive re-probe is implemented generically — `rk operator` is already a per-server singleton). The operator entry's argv presupposes `rk operator -L <server>` — the daemon-invocable form ([rk-riff](/run-kit/rk-riff.md) § Single-Quote Escaping and Task Injection): no `$TMUX` precondition, `-L`-addressed tmux calls, the window opened in a project main root derived from the server's user-role sessions (the main-worktree root carrying the deployed fab-operator skill; the home directory only as the last-resort fallback, with the kickoff miss then surfaced rather than silent), a singleton hit reported with `Operator tab already present.` and no `switch-client` (upt2, t7vy).
...
**Decision**: `respawn: [argv...]` on the entry runs via `exec.CommandContext` through the `Deps.RunRespawn` exec seam (90s timeout, home-dir cwd for the exec itself, daemon env); the operator entry says `rk operator -L {server}`, whose own launch derivation picks the operator window's directory (a skill-carrying project main root from the server's user-role sessions, home only as the launcher's fallback); session-resume stays the no-command default for session targets.
