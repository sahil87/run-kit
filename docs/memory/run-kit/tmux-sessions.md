...
*Read-side stays global*: only the destructive reap is scoped. `tmux.ListServers` / `/api/servers` / `IsGoTestServerName` are unchanged — foreign servers (including `rk-e2e-*`) remain visible in the UI by design, so this class of leak stays observable rather than masked.
...
## `rk reaper` — Operator Cleanup of Leaked Test Servers

`rk reaper` (since `260529-fww2-rk-reaper-command`) is a **top-level, operator-invoked** command — a sibling of `rk serve`/`rk riff`, registered via `rootCmd.AddCommand(reaperCmd)` in `cmd/rk/root.go`. It is **NOT** wired into any startup path and **NOT** nested under `serve`/`daemon`. It is a blunt janitor of last resort for test-server scaffolding that leaks into `/tmp/tmux-{uid}/` (Go test sockets reparent to init / lose cleanup races; PR #199 also left orphan `*.lock` recursion sockets). The command body is thin: `cmd/rk/reaper.go` only parses the `--dry-run` flag and renders the summary; **all** scan/probe/classify/reap logic lives in `internal/tmux/reaper.go` (Constitution §III).

### It does NOT replace the startup sweep

The reaper is orthogonal to `sweepOrphanedRelaySessions`. The startup sweep reaps `rk-relay-*` **sessions inside live servers** (see § Per-WebSocket Ephemeral Grouped Sessions) and stays untouched — `cmd/rk/serve.go` is unmodified by this change. The reaper reaps **whole test servers and dead/stale sockets**, and only when an operator runs it. Different scope, different trigger.

### Three leak shapes reaped

The reaper iterates **RAW** socket-dir candidates via `ScanSocketDir(ctx)` — **NOT** `ListServers`, which silently drops dead sockets (shape b) by probing them away. For each candidate it runs `probeServerAlive` and classifies into one action:
...
Classification is a **pure function** over `(name, probeAlive)` (no I/O, no real tmux) so the full matrix is unit-testable (`TestClassifyReap`). `ReapAction` is the exported enum: `ReapActionSkip` (zero value), `ReapActionKill`, `ReapActionRemove`. Branch order is load-bearing:

1. `.lock` suffix → `ReapActionRemove` (checked first — lock files carry no test prefix and reap regardless of probe state)
2. `name == ControlAnchorSessionName` (`_rk-ctl`) → `ReapActionSkip` (defense-in-depth, before the test-name branch)
3. `IsGoTestServerName(name)` → `ReapActionKill` if `probeAlive`, else `ReapActionRemove`
4. otherwise → `ReapActionSkip`

`IsGoTestServerName` is the single source of truth for test matching — the five prefixes (`rk-test-`, `rk-relay-test-`, `rk-verify-`, `rk-tmuxctl-test`, `rk-daemon-test`) are NOT re-declared in the reaper.

### Exclusions (never reaped)

- **`rk-e2e-*` sockets** — excluded for free: `IsGoTestServerName` omits the e2e prefix by construction (a live Playwright run needs its servers visible). No explicit reaper branch is required.
- **`_rk-ctl` control anchor** (`ControlAnchorSessionName`) — owned by `tmuxctl`; explicit skip branch (mirrors the startup sweep's anchor guard).
- **Any LIVE non-test server** — probe succeeds but the name doesn't match `IsGoTestServerName` → falls through to the final `ReapActionSkip`.

### No PID-liveness gate

There is no `kill(pid, 0)`, no parsing of `rk-test-<pid>-<ns>`. The operator invocation asserts "nothing live needs these," so all five test prefixes — **including the fixed-name `rk-tmuxctl-test` and `rk-daemon-test`** — and all `.lock` files reap unconditionally.

### I/O routine and the test seam

`ReapTestServers(ctx, dryRun) (ReapResult, error)` is the public I/O entry point: it scans via `ScanSocketDir`, then delegates per-candidate work to the internal seam `reapCandidates(ctx, dir, candidates, probe, dryRun)` — passing `socketDirPath()` and `probeServerAlive`. Tests drive `reapCandidates` directly with a temp dir + fake prober (no real tmux server spawned; kill is exercised only via `classifyReap`). `ReapResult` carries `Killed []string`, `RemovedSockets []string`, and `DryRunPlan []ReapPlanEntry` (`{Name, Action}` pairs, populated only on dry-run).

### UX and partial-failure contract

- **Default** reaps and prints a summary — total count plus the names of killed live servers and removed dead sockets/lock files; a zero-candidate run prints `Nothing to reap.`
- **`--dry-run`** lists each actionable candidate labeled by its would-be action (`kill`/`remove`) and mutates nothing — every entry remains on disk.
...
- `app/backend/internal/tmux/tmux.go` — `serverArgs()`, `tmuxExecServer()`, `ListSessions()`, `ListServers()` (delegates the raw socket-dir scan to `ScanSocketDir` and the probe to `probeServerAlive`), `ScanSocketDir(ctx)`, `socketDirPath()`, `filterSocketEntries()`, `probeServerAlive(ctx, name)`, `IsGoTestServerName()`, `ListKeys()`, `KillServer()`, `CreateSession()`, `SelectWindow(windowID, server)`, `ResolveWindowSession()`, `resolveWindowSessionIndex()`, `MoveWindow(windowID, dstIndex, server)`, `MoveWindowToSession(windowID, dstSession, server)`, `ReloadConfig()`, `EnsureConfig()`, `ConfigPath()`
- `app/backend/internal/tmux/reaper.go` — reaper logic: pure `classifyReap(name, probeAlive) ReapAction` (enum `ReapActionSkip`/`ReapActionKill`/`ReapActionRemove`), `ReapResult`/`ReapPlanEntry`, the public `ReapTestServers(ctx, dryRun)`, and the test seam `reapCandidates(ctx, dir, candidates, probe, dryRun)` (log-and-skip per entry via `slog`, aggregate error at end). Tested in `reaper_test.go`
- `app/backend/cmd/rk/reaper.go` — thin `reaperCmd` (top-level, `--dry-run` flag); calls `tmux.ReapTestServers` and renders the summary (`renderReapSummary`/`renderDryRun`); no scan/probe/remove/kill in `cmd/rk`
...
- `app/backend/internal/tmux/main_test.go`, `app/backend/api/main_test.go` — per-package `TestMain` dead-PID pre-sweep (since `260529-wtg4-isolate-relay-sweep-test-leaks`): before `m.Run()`, scans `/tmp/tmux-<uid>/` and `kill-server`s any `rk-test-*` / `rk-relay-test-*` socket whose embedded PID (2nd hyphen field of `rk-<kind>-<pid>-<ns>`, parsed by `parseTestSocketPID`) is dead per `testPIDAlive` — self-healing SIGKILL/panic/OOM residue from a prior run. Live-PID sockets (concurrent `go test`) and fixed-name `rk-daemon-test` / `rk-tmuxctl-test` (no parseable PID) are never touched. Logic is duplicated across the two packages (Go `_test.go` symbols are package-private); kills use `exec.CommandContext` + 5s timeout (constitution I)
- `scripts/test-e2e.sh` (`cleanup`/`trap cleanup EXIT`), `app/frontend/tests/e2e/global-teardown.ts` — e2e teardown now globs **all** `rk-e2e*` sockets (not just literal `rk-e2e`) in `/tmp/tmux-<uid>/`, best-effort, so timestamp-named secondaries (`rk-e2e-multi-*`, `rk-e2e-coupling-*`) don't leak on crash/Ctrl-C when a spec's `afterAll` never ran (since `260529-wtg4-isolate-relay-sweep-test-leaks`)
