---
type: memory
description: "Cron scheduling substrate — the `rk cron` CLI family (add/edit/list/rm/mute/pin/tick) over per-server intent files under $XDG_STATE_HOME/run-kit/cron/; stateless evaluator (every / backoff anchor-join / cron exprs + catch_up / wake_on; muted flag/lease); daemon Ticker; three-policy deliver axis (immediate / when-idle / skip-if-busy); any-role + session targets; if_absent dispositions; orphan TTL GC; circuit breakers; HTTP API; watchlist reader."
---
# Cron
...
| Verb | Behavior |
|---|---|
| `add <prompt>` | Records one entry via `cron.Add`; exactly one schedule flag — `--every <dur>`, `--idle-every <dur>` (sugar for a flat backoff ladder, min = max), bare `--backoff`, or `--cron "<expr>"`; creator and default target auto-captured inside a pane (the role → session → pane ladder), or an explicit `--role`/`--session`/`--pane` flag; repeatable `--respawn <arg>` carries the caller-supplied respawn argv (§ Caller-Supplied Respawn) |
| `edit <id>` | Replaces one entry's schedule / `deliver` / name / `if_absent` / respawn argv in place via `cron.Edit` (§ In-Place Edit) — each flag given replaces that field, omitted fields keep their values; the schedule flags are `add`'s four-way-exclusive set via the shared `cronScheduleFromFlags` parser (zero schedule flags keeps the schedule); target and creator are immutable; a schedule or deliver change appends one `rescheduled` log line (the anchor reset); unknown id ⇒ `no entry <id>`, exit 1 (9aup) |
| `list [--json]` | One row per entry — id, name, schedule summary, target, deliver, flags, last-fired — or the same records as a JSON array whose `schedule` is a structured object and which carries the intent fields `wake_on`, `if_absent`, `respawn` (plus `schedule_summary`, the table's rendering) |
| `rm <id>` | Removes one entry via `cron.Remove` |
| `mute <id> [--for <dur>] [--off]` / `pin <id> [--off]` | Set (bare) or unset (`--off`) the flag via `cron.SetMuted` / `cron.SetPinned`; `mute --for <dur>` leases the mute until now+dur via `cron.SetMuteLease` |
...
**`--json` intent fields are structured and keyed like the entry file** (`cronListRecord` + `cronListSchedule`/`cronListWakeOn` in `cron_list.go`, snake_case throughout — the HTTP API's camelCase projection is a separate type by design):
...
**Watchlist reader** (`internal/cron/watchlist.go`): `ReadWatchlist(path)` tolerantly parses the fab-owned operator state file (path via `FabOperatorStatePath` over the socket-path slug — § External Contracts) into `[]WatchlistEntry{ChangeID, Pane, Repo, Session, Stage, Agent, Branch, Kind}` (sorted by ChangeID) plus `last_tick_at`. The primary shape is fab-kit ≥ 2.25's top-level `tracked:` list (`parseTrackedItems`): an item is an entry iff it is a map with a string `id`, a map `scope` carrying a non-empty `pane` (the join key), and a null `done_at` — `ChangeID` is the item id, the six scope fields map one-to-one, `Kind` is the item's `kind` (never a filter — any pane-bearing item is watched); pane-less items (queued fab-change, github-pr, shell, task, note), done-but-unacked items, and malformed items (non-map element, missing id, non-map scope) are skipped, `paused` items are kept, and a non-list `tracked:` yields no entries. The pre-2.25 `monitored:` map (`parseMonitoredMap`, `Kind` empty) is read ONLY when the `tracked` key is absent — a one-release dual read; with `tracked` present, even as `[]`, the map is ignored. Unknown keys ignored, absent/corrupt file ⇒ `(nil, 0, false)`, never an error. rk never writes the file. `DefaultWatchlistStaleThreshold` (15m) is the UI-facing "is the monitoring system dead" window over `last_tick_at` — a display threshold only; nothing in cron gates a fire on it. The sessions-payload join this reader feeds is documented in [tmux-sessions](/run-kit/tmux-sessions.md) § Fab-Tier Derivation.
...
- **Seed interaction** — `EnsureRoleEntry` never rewrites an existing entry's schedule (idempotent; only the narrow respawn-argv / debounce backfill), so an edited operator tick survives every later `rk operator` launch: `rk cron edit <op-id> --idle-every 3m` is the supported way to turn the seeded tick into an idle reminder.
...
- `target` — `kind: role | session | pane` + the discriminant field (`role`, `session`, `pane`); a role target accepts ANY `@rk_win_role` value (`operator` is the predominant one; `RoleOperator` exists for the call sites that name the operator role literally, e.g. the `rk operator` seed); pane targets must pass `tmux.ValidPaneID` (the `%N` grammar).
...
`EnsureRoleEntry(dir, slug, spec)` is the idempotent seed helper: it scans the server's entries for one whose target is `{kind: role, role: spec.Target.Role}` — a hit returns the existing entry with `created=false`, a miss calls `Add` (`created=true`). The one mutation a hit allows is the narrow upgrade: when the matched entry has `if_absent: respawn`, an empty `respawn` argv, and the spec's argv is non-empty, the argv is filled from the spec and the file rewritten (the marshal drops the retired keys as a side effect) — every other field (min/max, muted, pinned, name, payload) is the user's tuning and is never reconciled. The role target is the idempotency key, not any field value (upt2).
...
**`rk cron list --json` is a fab-kit read contract.** fab-kit's operator clock (`operator_clock.go`) runs `rk cron list --json` to find the operator-tick entry — the row whose `target` equals `role:operator`, name `operator tick` as the tiebreak — and reads `id`, `name`, `target`, `muted`, `muted_until` to drive `rk cron mute`; its derived-schedule reconcile compares the structured `schedule` (`kind`/`interval`/`min`/`max`) against what it intends before applying a change through `rk cron edit` (§ In-Place Edit), whose schedule flags are `add`'s and so write the same `kind`/`interval`/`min`/`max` vocabulary the list emits (`--idle-every <d>` lands as `{kind: backoff, min: d, max: d}`). Those keys and the `target` summary string are therefore cross-repo stable; `schedule_summary` is the one field with a declared removal (§ CLI) (r5ao).
...
- **THEN** the schedules are exactly `{"kind":"every","interval":"1h0m0s"}`, `{"kind":"backoff","min":"1m0s","max":"30m0s"}`, and `{"kind":"cron","expr":"0 9 * * *","catch_up":"once"}`
...
`cron.ReadWatchlist` SHALL parse the fab operator state file's `tracked:` list tolerantly — an absent or corrupt file degrades to `(nil, 0, false)`, never an error; a non-list `tracked:` yields no entries; an item missing a string `id`, a map `scope`, or a non-empty `scope.pane` SHALL be skipped (nothing to join against), and an item whose `done_at` is set SHALL be skipped; a `paused` item SHALL be kept; `kind` SHALL not filter. When the `tracked` key is absent the legacy `monitored:` map SHALL be parsed as before (one-release dual read); when it is present the map SHALL be ignored. rk SHALL never write the fab-owned file.

#### Scenario: Tracked list read
- **GIVEN** a state file whose `tracked:` list holds a `fab-change` item with `scope.pane: '%180'` and `done_at: null`, a queued item with `scope.pane: null`, and an item with `done_at` set
- **WHEN** `ParseWatchlist` runs
- **THEN** exactly the first item is returned, with `Kind: fab-change` and its scope fields
- **AND GIVEN** `monitored: {legacy: {pane: "%1"}}` beside `tracked: []`, **THEN** no entries are returned
...
**Decision**: `respawn: [argv...]` on the entry runs via `exec.CommandContext` through the `Deps.RunRespawn` exec seam (90s timeout, home-dir cwd, daemon env); the seeded operator entry says `rk operator -L {server}`; session-resume stays the no-command default for session targets.
...
**Why**: the spec's notification deep-links to the console's log tab, not to a specific entry's detail sheet; the pure/seam split keeps `internal/cron`'s exported surface minimal and each half unit-testable through its existing fake.
...
**Why**: one line per gap (not per tick) bounds the occurrence walk near now; the console's `Cron Log` tab surfaces visible missed history; a diagnostic alone repeats every tick and leaves the anchor stale.
...
**Decision**: A tracked item is a watchlist entry exactly when it carries a `scope.pane` and its `done_at` is null; `paused` and `kind` do not affect inclusion. The legacy `monitored:` map is read only when the `tracked` key is absent — `tracked: []` beside a stale map yields nothing.
**Why**: The UI's question is "is the operator watching this pane": a done-but-unacked item would pin a watched row onto a finished or dead pane, a paused item is still in the tracked set, and only `fab-change` carries a pane probe today so a kind filter would encode a fab-kit detail. fab deletes `monitored` in the same write that introduces `tracked`, so both-present is residue; falling back on an empty list would resurrect stale entries after the operator removes the last item.
**Rejected**: filtering to `kind: fab-change`; keeping done items until `track rm`; merging both arms or falling back on emptiness; reading `fab operator track list --json` via a subprocess per fetch (a `fab` dependency on a per-request hot path for data already on disk).
