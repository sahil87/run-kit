...
// quitApp asks the running app to quit gracefully via AppleScript, addressing
// the name of the bundle that probed live at the swap boundary (the current
// name, or the legacy pre-rename one when it is the running install). A
// graceful quit (vs pkill) matters twice over: Electron gets its shutdown
// hooks, and the shell's window `close` handler captures lastPath so the
// relaunch restores the user's route.
