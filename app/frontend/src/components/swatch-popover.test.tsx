...
describe("SwatchPopover", () => {
  ...
  describe("banded Label picker", () => {
    ...
    it("the flair band lists the 14 states in display order, rain/scan leading, troy last", () => {
      renderLabelPicker();
      const cells = Array.from(
  ...
  describe("keyboard navigation (plain-grid bands)", () => {
    ...
    it("the marker row walks all 8 states; the flair rows walk 7 columns each (rain/nyan/… over scan/naruto/…)", () => {
      ...
      // Row 2 walks right: naruto, pacman, aquarium, invaders, warp, troy — clamped.
      for (const state of ["naruto", "pacman", "aquarium", "invaders", "warp", "troy", "troy"]) {
        arrow("ArrowRight");
        enter();
        expect(onSelectFlair).toHaveBeenLastCalledWith(state);
      }
      ...
      arrow("ArrowDown");
      enter();
      expect(onSelectFlair).toHaveBeenLastCalledWith("troy");
