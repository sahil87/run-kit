...
| [Cron Schedule Kinds](../wiki/cron-schedule-kinds.html) | Animated explainer for the [Cron](cron.md) spec's four firing mechanisms — `every` (interval from last delivery, outage → immediate catch-up), `cron` (wall-clock occurrences, missed-skips vs `catch_up: once`), `backoff` (the 1→30m doubling ladder and the anchor-join signal: a flip inside the 120 s attribution window is clock-caused, anything else resets), and `wake_on` (actionable transitions, self-exclusion, debounce hold) — plus the operator entry that combines backoff + wake. Canvas timelines with Play/Restart and rule toggles; loads finished. Self-contained; open in a browser |
