...
### OSC 8 hyperlink passthrough: the `hyperlinks` terminal-feature belongs in the conf, not in `forceTERM`
**Decision**: the managed conf appends the feature for the relay's `TERM` — `set -as terminal-features ',xterm-256color:hyperlinks'` — beside the existing `sync` entry, in `configs/tmux/default.conf` (the embed), `poweruser.conf`, and `byobu.conf`. `simple.conf` does not get it. The relay's `forceTERM` is untouched.
**Why**: tmux re-emits a pane's OSC 8 hyperlink to an attached client only when that client's terminal advertises the `hyperlinks` terminal-feature. No terminfo entry on the box carries `Hls`, and tmux's built-in `xterm*` defaults (`clipboard:ccolour:cstyle:focus:title`) omit it — so every hyperlink was silently stripped on the way to the dashboard, and bare URLs appeared clickable only because the frontend's regex addon re-discovered them independently ([ui/terminal](/run-kit/ui/terminal.md) § OSC 8 Hyperlinks). `terminal-features` is a tmux **server** option: `forceTERM` (`api/terminals_ws.go`) can only set environment on the relay PTY and no environment variable carries it, so the conf is not the tidier home but the **only possible** one. The append form is mandatory — a replacing form would drop `sync` and `extkeys`. Measured on a relay-equivalent PTY attach: 0 OSC 8 sequences reached the client before the line, 4 after, with the hyperlink present in tmux's grid in both runs.
**Rejected**: setting it imperatively on every dial the way `SetExitEmptyOff` is (§ Managed tmux.conf, and [tmux-sessions](/run-kit/tmux-sessions.md)) — the precedent exists but duplicates machinery already on the critical path: changing the conf body changes its SHA-256 stamp, so daemon start classifies it managed-stale, force-writes the embed, and `RefreshSweep` reloads every live managed server, with `reloadConfigForAttach` as a second path. Adding the line to `simple.conf` (it carries no TUI-compat block at all, sits on no code path, and declares `default-terminal "tmux-256color"`; the directly analogous call was already made by `260810-j93s`). Consequences that follow from the delivery vehicle, not defects: a **hand-edited** `~/.config/run-kit/tmux.conf` is never written by rk and will not pick the line up (the doctor row is its surface), and **external** servers receive rk's conf only through adopt. A non-xterm.js client attaching to the same server with `TERM=xterm-256color` from a terminal that genuinely lacks OSC 8 support now receives hyperlink escapes; unknown OSC sequences are ignored harmlessly by all but a very old or strict emulator.
