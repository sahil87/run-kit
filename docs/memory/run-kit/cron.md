...
`rk operator` seeds the operator-tick entry on every invocation via `EnsureRoleEntry` — unconditionally, before the command's singleton probe (seeding is disk-only, independent of tmux window state, so it cannot sit behind either early-return branch) and best-effort (a seed failure is one stderr warning, never a non-zero exit and never a skipped window-open — the snapshotter/ticker posture). Ownership is split deliberately: **rk defines the schema and the evaluator; the consumer (fab) seeds and tunes its entry** — `rk operator` still performs the seed today, and the rk-side seed's removal (leaving `rk operator` launcher-only) is pending fab-kit shipping its own seed (`rk cron add … --wake-on agent-state-change … --pinned`), tracked as backlog row `[pfo3]`. The seeded spec (`operatorTickEntrySpec`) is the cron spec's own operator-tick example (`docs/specs/cron.md` § Cron State): backoff 3m→24m keyed on the target pane's idle epoch, `wake_on: {agent-state-change, scope: server, debounce: 60s}` (the hold after the entry's own delivery — see § `wake_on` poll approximation), `target: {kind: role, role: operator}`, payload/name `"operator tick"`, `deliver: skip-if-busy`, `if_absent: respawn` with `respawn: ["rk", "operator", "-L", "{server}"]` (the `{server}` placeholder resolves to the stamped server at fire time), `pinned: true`, `created_by: {pane: $TMUX_PANE, at: now}` (`pane` empty under `-L`; the seed does not capture `created_by.session`). Existing servers are NOT migrated by rk — `EnsureRoleEntry`'s narrow upgrade is unchanged (the respawn-argv backfill and the below-spec debounce raise only), so an already-seeded entry keeps its stored schedule and deliver policy until fab's reconcile edits it via `rk cron edit`; the spec values reach fresh servers only. The server slug is the `-L/--server` value in server mode, else the caller-socket rule (`tmux.OriginalTMUX` → socket basename, the `cliServerLabel` helper) — see [rk-riff](/run-kit/rk-riff.md) § Single-Quote Escaping and Task Injection for the command side (upt2, ntde).
