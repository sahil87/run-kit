...
- **Optional split** — when `--split <setup-cmd>` is non-empty, a trailing `tmux split-window -h -c <worktree-path> "<setup-cmd>; exec \"${SHELL:-/bin/sh}\""` (built by the shared `shellWrap` helper) creates a horizontal split on the just-created window. The post-setup shell is the user's `$SHELL` (falling back to `/bin/sh`), not a hardcoded `zsh`. Split is skipped entirely when `--split` is empty/unset or when `new-window` failed.
