...
import { CronListView } from "@/components/cron-list";
 ...
 * this section is the operator-less cron home: the same Cron List body the
 * operator route's tab mounts (no `inline` — the modal detail-sheet variant),
 * under the staleness banner, over this section's single `useCronData`
 * subscription. Rendered whenever the mobile Server
 * page renders, operator or not — a deterministic projection of
 * `useCronData(server)`.
 *
 * The section scrolls itself into view once per server when the location hash
 * is `#cron` (the seam arm's navigation target) — keyed on the server, not
 * only on mount, because the Server page stays mounted across a `/one#cron`
 * → `/two#cron` param change. Explicit rather than relying on router hash
 * handling: the section lives inside the tiles' own scroll container, not the
 * document scroller.
 */
export function CronZone({ server }: { server: string }) {
  ...
  const scrolledForServerRef = useRef<string | null>(null);

  useEffect(() => {
    if (hash !== CRON_ZONE_HASH || scrolledForServerRef.current === server) return;
    scrolledForServerRef.current = server;
  ...
  return (
    <section ref={rootRef} id={CRON_ZONE_HASH} data-testid="clock-zone-cron" className="flex flex-col">
      ...
      {/* The view, not `CronList`: this section already holds the one cron
          subscription (the heading reads it), so the list shares it. */}
      <CronListView server={server} entries={entries} />
    </section>
  );
}
