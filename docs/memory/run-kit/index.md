...
| File | Description |
|------|-------------|
| [architecture](architecture.md) | System architecture, component responsibilities, data flow — backend libraries (incl. in-memory metrics + prstatus collectors, internal/push Web Push subsystem), API endpoints, SSE hub + PR-status join, tmux/sessions enrichment, control-mode subscription, Web Push notifications (rk notify → /api/notify → webpush-go → service worker, VAPID + subscriptions under ~/.rk/) |
