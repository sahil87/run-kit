...
type RenderOpts = {
  ...
  /** Supply the marker write seam. Omitted by default, which IS the board
   *  route's shape: rows then carry no marker strip and no Marker card row. */
...
describe("Sidebar — server-group header action cluster (x4sf)", () => {
  ...
  it("palette toggle opens the color + flair SwatchPopover portalled to document.body", async () => {
    ...
    // Label picker (color + flair, no marker band) — distinguished from the SERVER
...
// The marker write seam is an optional Sidebar prop: the terminal route wires
// it, the board route mounts the same Sidebar without it. Rows gate BOTH marker
// affordances — the fine-pointer strip target and the card's Marker row — on
// that one handler, so a board row can display a marker but never set one.
describe("Sidebar — marker seam is route-scoped", () => {
  it("omitting onWindowMarkerChange (the board route) renders no marker strip", () => {
    renderSidebar({ currentServer: "primary" });

    expect(screen.queryByTestId("marker-strip")).toBeNull();
    expect(screen.queryByTestId("marker-pad-anchor")).toBeNull();
  });

  it("wiring onWindowMarkerChange (the terminal route) renders the strip on every row", () => {
    renderSidebar({ currentServer: "primary", onWindowMarkerChange: vi.fn() });

    expect(screen.getAllByTestId("marker-strip").length).toBeGreaterThan(0);
  });
});
