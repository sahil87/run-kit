...
| D15 | **A `hexokit` GitHub account owns the brand repos, starting with the site.** `hexokit-site` transfers from `sahil87` to the **personal account `hexokit`** now (2026-09-12); GitHub redirects the old URL. The account becomes an **organization later** (conversion is one-way and fiddly — not urgent), and only then do other repos migrate. Until that conversion nothing else moves: the product repo stays `sahil87/run-kit` (→ `sahil87/hexokit` at R2 unless the org exists by then — decide at R2), the tap stays `sahil87/tap`, and the `shll.ai` stub stays under `sahil87`. Profile: Name `HexoKit`, URL hexokit.com, bio from the landing tagline, the cube-in-hexagon avatar | Confirmed | Sahil: personal account now, org later; moving the site is enough for now. Transfer checklist: re-add the `hexokit.com` custom domain + **account-level domain verification** under the new owner (Pages verification is per-account — S2's open manual step moves with it); confirm the deploy + both Refresh workflows still have `contents: write` under the new owner; add a third `gh auth` login for `hexokit`; update the plan's `sahil87/hexokit-site` links (S1–S5, X1) — GitHub redirects them meanwhile |
