...
func TestCaptureNodeRealTreeSelfExcludesAndDepth(t *testing.T) {
	...
	// The mux family is captured to full depth with exactly its ten members;
	// snapshot carries its three children at depth.
	mux, ok := childByName(n, "mux")
	if !ok {
		t.Fatal("mux should be present in the real tree")
	}
	for _, name := range []string{"send", "await", "capture", "kill", "process", "panes", "reap", "snapshot", "init-conf", "guard"} {
		if _, ok := childByName(mux, name); !ok {
			t.Errorf("mux should have its %q subcommand captured", name)
		}
	}
	if len(mux.Commands) != 10 {
		t.Errorf("mux has %d captured subcommands, want exactly 10 (send, await, capture, kill, process, panes, reap, snapshot, init-conf, guard)", len(mux.Commands))
	}
