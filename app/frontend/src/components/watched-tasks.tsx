...
/**
 * The operator watchlist segment body — the console's Operator Tasks segment
 * (desktop drawer, dense) and the mobile operator route's `?tab=tasks` content
 * slot. A pure projection over the sessions payload taken by PROP: no hook, no
 * fetch, no timer — every relative age is computed at render from `Date.now()`
 * and the sessions SSE cadence is the clock. Never an error state: absent
 * facets degrade to hint lines.
 *
 ...
 * age or `no recent tick` when no session carries a tick; (2) the scroll body
 * holding the shared `WatchedTable` (the same six columns the Server page's
 * WATCHED zone renders) when rows exist; (3) otherwise a centered hint line —
 * `no server resolved` / `No operator on this server` / `No watched workers`
 * (an absent state file reads as an empty watchlist by the backend's tolerant
 * parse, so an empty list is a state, not an error).
 */
