---
type: memory
description: "The rk MCP surface — the `rk mcp` stdio server and the daemon's `/mcp` streamable-HTTP route over `internal/mcp`: the compiled-in policy table (a verb with no row is not a tool), Cobra-introspection schemas with the `Resolve` drift guard, the argv `Executor` (stdin, 45 s cap, `TMUX` stripped), the three-tier JSON result parser, twelve seeded tools incl. the `board` action-enum row and the `operator_request` Talk row, the SDK transport behind the `OriginPolicy` allowlist, the `rk doctor` rows, `rk url --mcp`, and the E2E harness."
---
# rk MCP Surface
