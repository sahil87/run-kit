---
description: "The app/desktop Electron viewer shell — a BrowserWindow client of an existing rk serve URL that never spawns or supervises the daemon. Covers the servers.json store, welcome flow + main-process health ping, the ⌘-tier accelerator-avoidance menu seam (⌃1–9 switcher, ⌘W unbound), the window.runkitShell preload bridge + the SPA's isShell() seam, security wiring (sandbox, navigation/permission allowlists, senderFrame-gated IPC), and ad-hoc per-arch DMG packaging with git-describe version injection."
type: memory
---
...
- **Ad-hoc signing only**: `identity: null` — electron-builder ad-hoc-signs automatically (arm64 macOS refuses to launch fully unsigned binaries); no Developer ID, no notarization. The accepted cost is Gatekeeper friction on first launch ("Open Anyway", or `xattr -dr com.apple.quarantine` on Sequoia's "damaged" variant).
- **Version injection**: `package.json` carries a `0.0.0` placeholder; the real version rides `electron-builder --config.extraMetadata.version=$VERSION` (rewrites the packaged package.json → `app.getVersion()` → `runkitShell.version`). `scripts/build-desktop.sh` derives `$VERSION` from `git describe --tags --abbrev=0` (leading `v` stripped, fallback `0.0.0-dev`) — the git tag is the version source; the desktop pipeline reads no VERSION file and is independent of `scripts/build.sh`.
...
DMG builds are **local-only** (`just build-desktop` on a Mac): CI attachment of DMGs to GitHub Releases is a separate change, tracked in `fab/backlog.md` `[5uae]`.
...
**Decision**: Two DMGs (arm64, x64) with `identity: null`, versioned via `--config.extraMetadata.version` from `git describe`.
**Why**: Mirrors the `rk-darwin-{arm64,amd64}` artifact convention; a universal binary doubles size for no benefit; notarization is explicitly not a requirement, and ad-hoc satisfies arm64's signing floor; the git tag is the repo's version source, so the packaged version needs no committed file.
