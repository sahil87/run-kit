...
- [x] A-011 R1: the allowlist has exactly 29 rows, the two action-enum tools are `tab_web` and `board`, `mux kill --force` is stated as not exposed, and the `answer` key enum is listed verbatim (30 data rows counted)
