---
type: memory
description: "The GUI surface — the gui.enabled switch and gui.wm pin, the rk gui family (on/off/status/env/restart/exec/launch/open; agent verbs windows/focus/click/move/scroll/type/key/shot/wait/clip; lock/unlock; supervise), the allowlisted launcher (+ --cdp), the rk-gui supervisor (Xvnc argv, icewm WM ladder, seeded profile, @rk_gui_* stamps), the /ws/gui RFB relay (observe mode, human-input timestamp), the event: gui slot, /api/gui/* routes, internal/gui, doctor row, rk skill gui, C5/C6 verdicts."
---
# GUI Surface
...
The GUI surface exposes the host's desktop as a fourth tile beside `tty`/`code`/`web`. This file covers the backend half plus the agent half: the `gui.enabled` switch and `gui.wm` pin, the `rk gui` CLI family (the display verbs, the agent verbs — `exec`/`shot`/`launch` plus the windows/input/wait/clip/open/lock set — and the hidden `supervise`), the `rk-gui` supervisor session, the RFB-over-WebSocket relay, the `event: gui` state slot, the `/api/gui/*` routes, the doctor row, the `internal/gui` package, the `rk agent setup` gui display block, and the `rk skill gui` topic page. The frontend tile is a separate surface (design authority: `docs/specs/gui.md` and the plan `fab/plans/sahil/26-09-09-gui-surface.md`). (fkh1) (bbv1) (2jl3) (d0za)
...
`cmd/rk/gui.go` plus one file per verb cluster (`gui_exec.go`, `gui_shot.go`, `gui_launch.go`, `gui_open.go`, `gui_window.go`, `gui_input.go`, `gui_wait.go`, `gui_clip.go`) — a cobra parent in the `code-server` family's shape (Use/Short/Long with a Subcommands list), all members registered unconditionally on `rootCmd`:
...
No agent-facing verb (`env`, `exec`, `shot`, `launch`, `open`, `windows`, `focus`, `click`, `move`, `scroll`, `type`, `key`, `wait`, `clip`, `lock`, `unlock`) SHALL write `gui.enabled`; a gated refusal exits 1 with the shared hint and starts no process and writes no file. The switch remains the user's alone — the "Never on by default" rule extended to the agent surface. (bbv1) (d0za)
...
Every verb that acts on the live display (`exec`, `shot`, `launch`, `open`, `windows`, `focus`, `click`, `move`, `scroll`, `type`, `key`, `wait`, `clip`, `lock`, `unlock`) SHALL apply the same gate — the macOS refusal first, then enabled, then reachable — and `env` SHALL apply the enabled/reachable half, all through the shared `guiDarwinRefusal`/`guiRequireReachable` helpers and the named `guiErrOff`/`guiErrNotRunning` constants, so every gated verb prints byte-identical refusals for the same state. (bbv1) (2jl3) (d0za)
