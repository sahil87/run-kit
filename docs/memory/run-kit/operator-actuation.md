...
service. Everything lives in `app/backend/api/operator.go` (handlers + registry),
the routes registered in `api/router.go` beside the chat routes. Nothing in any
...
Both handlers SHALL deliver the rendered prompt through the shared
`deliverOperatorPrompt` seam (so the two cannot drift), which delivers
in-process via
