...
**Window rows**: Two-line layout — activity dot + window name + right-side info (fab stage, duration, info button, peek toggle, color picker, kill) on line 1, plus an optional always-visible last-line preview below it when `win.lastLine` is non-empty (`text-xs text-text-secondary truncate min-w-0`). Rows with no captured output keep their single-line height — no reserved space, no placeholder. All rows have `border-l-2` (transparent when not selected to prevent layout shift). Currently selected window highlighted with `bg-accent/10` + `border-accent` + `font-medium` + `rounded-r`. Click on the row navigates to `/:session/:window`.
...
6. **Live last-line preview (always visible)** — When `win.lastLine` is a non-empty string, `window-row.tsx` renders a single-line element beneath the window-name row styled `text-xs text-text-secondary truncate min-w-0`. The value comes from the SSE payload's `WindowInfo.lastLine` field (populated server-side; see architecture.md). Missing/empty `lastLine` emits no DOM element — the row collapses to its single-line layout. The frontend uses type narrowing (`if (win.lastLine) { ... }`) rather than casts.

7. **Peek expand/collapse toggle** — A chevron button (`\u25B8` collapsed / `\u25BE` expanded) sits inside the existing `absolute right-2` hover-reveal cluster, visually before the kill/color buttons. Uses the established `opacity-0 group-hover:opacity-100 coarse:opacity-100` hover-reveal pattern (always visible on touch, hover-revealed on pointer devices). The button sets `aria-expanded` to the current state and a dynamic `aria-label` of `Expand output peek for {window.name}` / `Collapse output peek for {window.name}`. Clicking calls `stopPropagation` so the underlying row click (window navigation) does not fire. Multiple windows can be expanded simultaneously.

8. **Expanded peek block** — When `isExpanded`, `window-row.tsx` renders a block below the row styled `font-mono text-xs text-text-secondary bg-bg-card px-2 py-1`, with each line CSS-truncated to the sidebar width. A local state machine (`idle | loading | ready | error`) drives the content:
   - On expand, issues `capturePane(session, win.index, 3)` via the API client; renders `Loading\u2026` in `text-text-secondary` while in flight.
   - On success, renders up to 3 lines in sequence.
   - On failure, renders `Unable to load output` inline in `text-text-secondary` — no toast is shown (peek errors are scoped).
   - While expanded, a `useEffect` keyed on `[isExpanded, win.lastLine]` issues a new `capturePane` when the SSE-delivered `lastLine` transitions to a new non-empty value. Empty/undefined `lastLine` transitions are suppressed — the last-known peek content remains visible through transient capture failures.
   - A single-flight guard (`inFlightRef` + `staleRef`) ensures at most one capture per window at a time; if `lastLine` changes again during a fetch, exactly one trailing re-fetch runs after the current settles. No `setInterval`-based polling anywhere.
   - On collapse, local state is discarded.

**Peek expansion state** — lifted to `sidebar/index.tsx` as `expandedPeeks: Set<string>` keyed by `${session}:${windowId}` (stable tmux `@N` id, not the mutable index). The stable key means reorders and renames preserve expansion state across SSE ticks. State is in-memory React only — no `localStorage`, no `sessionStorage`, no URL persistence, no backend store. Page reload resets all windows to collapsed.
...
| 2026-04-17 | Inline output peek and live activity previews — Sidebar window rows gain an always-visible last-line preview beneath the name row (sourced from SSE-delivered `win.lastLine`, rendered only when non-empty, `text-xs text-text-secondary truncate min-w-0`) plus a chevron expand/collapse toggle (`\u25B8`/`\u25BE`) in the existing `absolute right-2` hover-reveal cluster. Expanded peek renders a `font-mono text-xs bg-bg-card px-2 py-1` block showing up to 3 ANSI-stripped lines fetched on-demand via `capturePane(session, index, 3)`; `Loading\u2026` placeholder while pending, `Unable to load output` inline on error (no toast). Expanded state lives in `sidebar/index.tsx` as `expandedPeeks: Set<string>` keyed by `${session}:${windowId}` (stable id — survives reorder). Peek refreshes on `lastLine` SSE changes via `useEffect` with empty-transition suppression (preserves last-known content on transient capture failures) and a single-flight guard (`inFlightRef` + `staleRef`). Toggle has dynamic `aria-label`/`aria-expanded` and `stopPropagation` so window selection doesn't fire. In-memory state only — no `localStorage`, reset on reload. All strings are rendered as React text children (no `dangerouslySetInnerHTML`). | `260405-7l79-inline-output-peek-live-previews` |
