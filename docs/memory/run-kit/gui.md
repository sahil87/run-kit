---
type: memory
description: "The backend half of the GUI surface — the gui.enabled registry switch, the rk gui CLI family (on/off/status/env/restart + hidden supervise), the rk-gui supervisor session (display selection, Xvnc argv, WM ladder, @rk_gui_* stamps on the =rk-gui: target, the spawn's XDG_STATE_HOME pin, stay-idle exit), the /ws/gui/{id} RFB relay (close-code gates, macOS view-only), the event: gui TTL-probed slot, /api/gui/* routes, the settings-POST side effect, the doctor row. Frontend: ui/lenses-and-layout.md."
---
# GUI Surface (Backend)
...
The GUI surface exposes the host's desktop as a fourth tile beside `tty`/`code`/`web`. This file covers the backend half: the `gui.enabled` switch, the `rk gui` CLI family, the `rk-gui` supervisor session, the RFB-over-WebSocket relay, the `event: gui` state slot, the `/api/gui/*` routes, the doctor row, and the `internal/gui` package. The frontend tile — the `GuiSurface` noVNC canvas, the `useGui()` context, the off-confirm — lives in [lenses-and-layout](/run-kit/ui/lenses-and-layout.md) § GUI Surface, [keyboard-and-palette](/run-kit/ui/keyboard-and-palette.md) § The `GUI:` palette family, and [dialogs-and-state](/run-kit/ui/dialogs-and-state.md) § GUI Off-Confirm Dialog; the agent verbs (`rk gui exec`/`shot`) remain a design target (design authority: `docs/specs/gui.md` and the plan `fab/plans/sahil/26-09-09-gui-surface.md`). The payload/routes documented below are the shipped contract — the spec's substrate diagram is written against these same keys (`enabled`, `backend`, `reachable`, `display`, `width`, `height`, `viewers`) and routes (`GET /api/gui/{id}`, `POST /api/gui/{id}/restart`, on/off via `POST /api/settings`). (fkh1, o2sp)
