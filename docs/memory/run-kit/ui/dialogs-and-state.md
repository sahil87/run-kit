---
description: "Component conventions; modal dialogs and popup surfaces (the spring-loaded marker pad, the GUI off-/restart-confirms on one shared shell with the gui.enabled off-interception and per-viewer rk-gui-*/rk-web-capture postures); the overlay-presence registry (modal + click-opened-menu transient counts — the hide signal for surfaces that cannot be painted over) and status-bar toasts; clipboard and e2e host-state rules; the Zustand window store, optimistic mutation feedback, recently-closed mirror."
type: memory
---
