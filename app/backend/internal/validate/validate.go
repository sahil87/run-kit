...
func ValidateColorValue(value string) string {
	...
	if len(parts) < 1 || len(parts) > 2 {
		return "Color must be a single index (0-15) or a blend (a+b)"
	}
	for _, p := range parts {
		...
		if n < 0 || n > 15 {
			return "Color indices must be between 0 and 15"
		}
	}
	return ""
}
...
// ("", false) when malformed. Used by tolerant-read storage paths (settings,
// run-kit.yaml) to accept int-or-string on read and always normalize to string.
func NormalizeColorValue(value string) (string, bool) {
	v := strings.TrimSpace(value)
	if v == "" {
		return "", false
	}
	if ValidateColorValue(v) != "" {
		return "", false
	}
	return v, true
}
