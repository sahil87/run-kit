...
**Layout**: Outer wrapper `flex-1 min-h-0 flex flex-col` with two regions: (1) pinned stats line (`shrink-0 px-4 sm:px-6 pt-4 sm:pt-6`) — `"{N} sessions, {M} windows"` (`text-sm text-text-secondary`, counts derived from the `sessions` array); (2) scrollable tile area (`flex-1 min-h-0 overflow-y-auto ...`). Only the grid scrolls.
...
Since `260701-vshd-cockpit-host-console-home`, `ServerListPage` (`app/frontend/src/components/server-list-page.tsx`, route `/`) renders a **HOST HEALTH** zone ABOVE the tmux-server tile grid, inside the existing scroll container (`div.flex-1.min-h-0.overflow-y-auto`). This makes `/` — the only route that is about the BOX, not a session — a machine console. All three zones now render in top-to-bottom order **HOST HEALTH → TMUX SERVERS → SERVICES** (each an uppercase-labelled `<section>`; the server-tile grid gained a "Tmux Servers" heading + inline server count, and SERVICES moved to last, after the tiles — `260701-5d0z-service-tiles-host-console`; see § SERVICES Zone on `/`). Read-only awareness view; nothing here mutates host state (Constitution IV).
