---
type: memory
description: "run-kit's configuration story: fixed root $HOME/.config/run-kit/ (no XDG_CONFIG_HOME), the internal/settings registry + 12-key inventory behind /api/settings (partial merge, all-or-nothing validation), override order (code default < config.yaml < env < CLI flag; env restricted to the RK_PORT/RK_HOST/RK_CODE_SERVER_PORT keys), value-home boundaries, the rk-owned hash-stamped managed tmux.conf (daemon-start refresh, @rk_managed-gated reload sweep + WS-attach reload), breadcrumb migrations, ~/.rk tenants, the cb/ code-bridge state tenant + profile-seeded rk.bridge.enabled."
---
# Configuration
...
**Why**: the user's own tmux.conf regime and rk's managed conf are different regimes — sourcing rk's conf into a server the user configured themselves silently changes its keybindings, styling, and options. tmux ignores `-f` on an already-running server, so the flag applies only at birth, which is exactly when the `@rk_managed` stamp lands — gating it would be dead code.
