...
- No config toggle / env var: the feature is armed exactly when an operator window exists on a server (degrade-to-absent razor). If field use shows it needs a switch, that's a follow-up.
...
| 5 | Confident | No config toggle — armed exactly when an operator window exists | Degrade-to-absent razor from the plan; Constitution VII (convention over configuration); a toggle is an easy follow-up | S:60 R:85 A:75 D:60 |
