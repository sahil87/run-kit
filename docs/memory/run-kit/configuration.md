---
type: memory
description: "run-kit config: fixed root $HOME/.config/hexokit/ via internal/apphome's dual-read rule (legacy run-kit dir for one release; RK_CONFIG_DIR still wins); release-start homemigrate copy + atomic publish with the legacy-port pin; the settings registry's 19-key inventory; override order code default < config.yaml < env < CLI flag; ports.env policy incl. DaemonLegacy pin + dormant doctor row; managed tmux.conf; localStorage hexokit- prefix + marker-guarded legacy copy; breadcrumb migrations."
---
# Configuration
...
**Why**: `RK_PORT` wins over the pin anyway, and capturing a transient `RK_PORT` (dev rig, test) would pin a bogus port permanently. `DaemonDefault` moves to 6123 in the C5 release.
